...
<details>
  <summary>Details</summary>
Motivation: 心理健康压力日益普遍，需要开发非侵入性诊断工具。EEG信号虽直接反映神经活动，但其非平稳性和高维度特性带来建模挑战。

Method: 采用混合架构（卷积层、LSTM层和注意力机制）处理EEG信号，结合预处理步骤（带通滤波、Z-score标准化和分段）。
...
<details>
  <summary>Details</summary>
Motivation: 结合SAR成像的操作特性，探索两种HAPS部署策略，以优化通信用户的总速率吞吐量，同时满足SAR成像要求。

Method: 提出两种HAPS部署策略（准静态和动态），并通过联合优化部署策略和3D波束成形，解决非凸优化问题。

Result: 数值结果表明所提方法优于基准方案。

Conclusion: 通过高效算法解决了非凸优化问题，验证了所提方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 增强平面物理信息安全，同时保持天线的紧凑性和全向性能。

Method: 使用两个紧密排列的印刷曲折线单极天线作为单一辐射单元，通过差分功率激励和实时动态切换实现方向调制。

Result: 实验验证了16-QAM和256-QAM传输下的窄E平面信息束（约34°和15°）和全向H平面信息束。

Conclusion: 该天线设计简单有效，能够提升平面物理信息安全性。
...
<details>
  <summary>Details</summary>
Motivation: 可移动天线因其能主动重构无线信道的特性，在无线通信中受到关注，研究旨在利用MA提升NOMA系统的性能。

Method: 采用连续凸逼近（SCA）和交替优化方法，联合优化波束成形和MA位置。

Result: 仿真显示，MA增强的NOMA系统在用户总速率上显著优于固定天线（FPA）系统和传统OMA系统。
...
<details>
  <summary>Details</summary>
Motivation: 在未知杂波环境下，双基地毫米波MIMO雷达系统需要高精度的目标参数估计方法，现有算法在离网格情况下性能不足。

Method: 利用角度-多普勒（AD）域的三维稀疏性，结合SBL框架估计目标参数（AoD、AoA和速度），并开发超分辨率离网格SBL框架递归更新参数网格。

Result: 仿真结果表明，所提算法性能优于现有方法，并能接近推导的Cramér-Rao界（CRB）和贝叶斯CRB。

Conclusion: 该方法在未知杂波环境下实现了高精度的目标参数估计，并通过离网格SBL框架显著提升了性能。
...
Conclusion: MMWiLoc为毫米波Wi-Fi定位提供了高效解决方案，并开源数据集和代码以促进研究。
...
<details>
  <summary>Details</summary>
Motivation: 研究STAR-RIS在多用户MIMO广播信道中的能效优化，特别是在有限块长度和严格延迟、可靠性要求下的表现。

Method: 通过最大化总能量效率（EE），分析了STAR-RIS在不同参数（如码字长度和最大容忍误码率）下的性能。

Result: STAR-RIS能显著提升能效，尤其在码字长度较短和误码率要求更严格时效果更明显。

Conclusion: STAR-RIS在延迟和可靠性要求更高的系统中更具能效优势。
...
<details>
  <summary>Details</summary>
Motivation: 物理场张量重建在无线通信和水下声学等应用中至关重要，但现有方法因观测数据有限且噪声大，需依赖先验信息。传统深度学习方法存在训练与测试阶段模型不匹配的问题。

Method: FieldFormer基于Tucker张量分解模型，引入注意力机制学习核心张量的稀疏模式，减少解空间，实现复杂度自适应的神经表示。

Result: 理论分析支持其可恢复性，实验证明其在多种物理场张量重建任务中优于现有方法。

Conclusion: FieldFormer通过自监督学习有效解决了传统方法的问题，为物理场重建提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的随机动态系统常偏离卡尔曼滤波的线性高斯假设，需要数据驱动的滤波技术。

Method: 使用递归Joseph公式传播误差协方差，并通过优化高斯负对数似然来训练模型。
...
Result: 仿真结果表明，所提方法在频谱效率上优于仅依赖带内信息或其他带外辅助的现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 无线用户和带宽需求的指数增长需要创新的通信范式，语义通信通过传输提取的意义而非原始比特，提高了频谱效率。

Method: 研究了SemCom与BitCom的集成，分析了多址技术（如NOMA）和多模态SemCom框架，并探讨了卫星网络中的应用。

Result: 语义通信在异构网络中展现出潜力，能够缓解带宽限制和恶劣信道条件。

Conclusion: 语义通信是下一代网络的有前景的解决方案，未来需进一步研究其在6G中的应用。
...
<details>
  <summary>Details</summary>
Motivation: 在频谱共享的卫星与地面网络中，缺乏共享信道状态信息时，如何减少卫星对地面系统的干扰是一个关键问题。

Method: 通过利用地面用户终端的分布信息建立干扰模型，提出了两种优化准则（加权和速率最大化和均方误差最小化）的鲁棒发射波束成形方案。

Result: 数值仿真验证了所提方案的有效性，加权和速率最大化方案实现了更高的可达速率，而均方误差最小化方案提供了低复杂度的闭式解。

Conclusion: 论文提出的方法有效解决了卫星与地面网络共享频谱时的干扰问题，并通过近似方法简化了干扰评估的复杂性。
...
### [28] [DMRS-Based Uplink Channel Estimation for MU-MIMO Systems with Location-Specific SCSI Acquisition](https://arxiv.org/abs/2506.11899)
*Jiawei Zhuang,Hongwei Hou,Minjie Tang,Wenjin Wang,Shi Jin,Vincent K. N. Lau*
...
<details>
  <summary>Details</summary>
Motivation: 随着多用户MIMO系统用户数量增加，需通过正交覆盖码（OCC）高效复用DMRS以减少干扰，但现有方法在性能和复杂度上存在不足。

Method: 提出SCSI辅助的贝叶斯估计器（SA-BCE）及其窗口版本（SA-WBCE），利用位置特定SCSI数据库和天线-频域窗口技术降低复杂度。

Result: 仿真表明，SCSI数据库构建方法精度高，SA-BCE和SA-WBCE性能显著优于现有方法。

Conclusion: 通过SCSI辅助和低复杂度设计，显著提升了多用户MIMO系统的信道估计性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的ASR后处理方法（ED-CEC）在识别罕见词时表现不佳，尤其是发音相似但拼写不同的词。此外，错误检测模块存在过度检测问题。

Method: 提出PMF-CEC方法，结合音素信息增强多模态融合，并引入保留概率机制过滤低置信度编辑操作。

Result: 在五个数据集上的实验显示，PMF-CEC进一步降低了错误率，尤其在处理同音词时表现更优，且推理速度合理。

Conclusion: PMF-CEC在罕见词识别和错误检测方面优于现有方法，具有更快的推理速度和更强的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 构音障碍和老年语音识别面临数据稀缺、分布不均和说话者异质性等挑战，同时隐私问题促使从集中式转向联邦学习。

Method: 采用参数、嵌入和损失三种正则化方法，在联邦学习过程中进行多层次的优化。
...
<details>
  <summary>Details</summary>
Motivation: 增强汽车系统智能，改善乘客舒适度和车辆安全，推动数据驱动的城市道路管理。

Method: 利用深度神经网络和Open AI生态系统的预训练模型，从车轮底座麦克风提取声学特征进行分类。

Result: 支持下一代汽车系统的商业案例，提升自动驾驶和辅助驾驶系统的适应性。

Conclusion: 该方法不仅优化了乘客体验和安全性，还为智能、可持续的城市交通管理奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有语音处理工具缺乏验证，可能导致行为相关信息的捕捉不可靠，影响模型的再现性和公平性，尤其是在不同人群和情境下。

Method: 通过评估OpenSMILE和Praat提取的语音特征，分析其在自闭症青少年中的可靠性。

Result: 不同工具提取的特征存在显著差异，影响了模型在不同情境和人口群体中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在解决儿童自然语音数据转化为可用信息的技术难题，推动语音科学的发展。

Method: 通过三年实验，改进语音类型分类任务的特征表示、架构和参数搜索，并分析数据相关性与数量的影响。

Result: 实验表明，特征、架构和参数搜索的改进仅带来边际性能提升，数据相关性和数量对进展更为重要。

Conclusion: 强调收集可共享的适当数据对语音技术发展的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 通过全天候录音捕捉儿童语言输入与输出，减少观察者偏差，提高研究有效性。

Method: 总结现有技术资源，分析自动化注释中的误差来源，提出数据质量评估指标。

Result: 指出完全自动化的质量控制不可行，但提供了改进数据收集和分析的实际策略。

Conclusion: 研究为儿童语言研究提供了实用的数据收集和分析建议，强调误差识别与质量控制的重要性。
...
### [53] [Intelligibility of Text-to-Speech Systems for Mathematical Expressions](https://arxiv.org/abs/2506.11086)
*Sujoy Roychowdhury,H. G. Ranjani,Sumit Soman,Nishtha Paul,Subhadip Bandyopadhyay,Siddhanth Iyengar*

Main category: eess.AS

TL;DR: 评估了五种TTS模型处理数学表达式（MX）的能力，发现其输出可懂性不足，且性能显著低于专家朗读。


<details>
  <summary>Details</summary>
Motivation: 目前对TTS模型处理MX的评估有限，需填补这一空白。

Method: 通过听写测试和用户评分（MOS）评估五种TTS模型，使用LLM将LaTeX MX转为英语发音，并量化可懂性。

Result: TTS模型对MX的输出可懂性差，性能显著低于专家朗读，LLM选择影响有限。
...
### [54] [Better Pseudo-labeling with Multi-ASR Fusion and Error Correction by SpeechLLM](https://arxiv.org/abs/2506.11089)
*Jeena Prakash,Blessingh Kumar,Kadri Hacioglu,Bidisha Sharma,Sindhuja Gopalan,Malolan Chetlur,Shankar Venkatesan,Andreas Stolcke*
...
Conclusion: 统一框架结合LLM后处理能有效提升ASR模型的训练效果。
...
### [55] [Tracking of Intermittent and Moving Speakers : Dataset and Metrics](https://arxiv.org/abs/2506.11145)
*Taous Iatariene,Alexandre Guérin,Romain Serizel*

Main category: eess.AS

TL;DR: 本文提出了一种针对间歇性和移动声源跟踪的问题，并介绍了LibriJump数据集和新的跟踪关联度量方法。
...
Method: 提出LibriJump数据集（包含声源静默期间位置变化的声学场景），并采用计算机视觉领域的跟踪关联度量方法。

Result: 实验表明关联度量与现有跟踪度量在连续和不连续轨迹中具有互补性。

Conclusion: 本文为解决不连续轨迹的跟踪问题提供了新的数据集和度量方法。
...
### [56] [Improved in-car sound pick-up using multichannel Wiener filter](https://arxiv.org/abs/2506.11157)
*Juhi Khalid,Martin Bouchard*
...
<details>
  <summary>Details</summary>
Motivation: 随着汽车电子和传感器技术的发展，多麦克风拾音在免提通话和语音命令应用中变得可行，但信号处理仍面临带宽和处理能力的限制。

Method: 采用多通道维纳滤波算法，针对车内双麦克风系统优化，评估其在多种噪声条件下的性能，并研究头部运动的影响。

Result: 实验表明，该方法在语音质量提升和背景噪声减少方面显著优于简单的麦克风信号混合。
...
### [57] [S2ST-Omni: An Efficient and Scalable Multilingual Speech-to-Speech Translation Framework via Seamlessly Speech-Text Alignment and Streaming Speech Decoder](https://arxiv.org/abs/2506.11160)
*Yu Pan,Yuguang Yang,Yanni Hu,Jianhao Ye,Xiang Zhang,Hongbin Zhou,Lei Ma,Jianjun Zhao*

Main category: eess.AS

TL;DR: S2ST-Omni框架通过分解语音到语音翻译任务为语音到文本和文本到语音，结合预训练模型，解决了高质量、低延迟翻译及对大规模平行语料依赖的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多语言语音到语音翻译面临高质量低延迟的挑战，且依赖难以获取的大规模平行语料。

Method: 将任务分解为语音到文本翻译和文本到语音合成，结合Whisper和Qwen 3.0预训练模型，引入轻量级语音适配器对齐表示，并使用流式语音解码器实现实时性能。

Result: 在CVSS基准测试中，S2ST-Omni优于现有方法，且保持低延迟。
...
### [58] [Advances in Small-Footprint Keyword Spotting: A Comprehensive Review of Efficient Models and Algorithms](https://arxiv.org/abs/2506.11169)
*Soumen Garai,Suman Samui*

Main category: eess.AS

TL;DR: 本文综述了小型足迹关键词检测（SF-KWS）的技术，探讨了七类方法，为开发高效TinyML框架提供了资源。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习的发展，SF-KWS在智能语音设备和IoT应用中需求增加，但边缘设备的资源限制需要高效解决方案。

Method: 研究了七类技术：模型架构、学习技术、模型压缩、注意力感知架构、特征优化、神经网络搜索和混合方法。

Result: 提供了SF-KWS领域的全面概述，并识别了潜在研究方向。

Conclusion: 本文为SF-KWS的研究和应用提供了宝贵资源，并指出了未来研究方向。
...
### [59] [Efficient Speech Enhancement via Embeddings from Pre-trained Generative Audioencoders](https://arxiv.org/abs/2506.11514)
*Xingwei Sun,Heinrich Dinkel,Yadong Niu,Linzhang Wang,Junbo Zhang,Jian Luan*

Main category: eess.AS

TL;DR: 本文提出了一种基于预训练音频编码器的语音增强方法，通过去噪编码器和声码器生成清晰语音，实验证明其优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 探索利用预训练模型的音频嵌入进行语音增强，以替代传统的时频掩蔽或信号预测方法。

Method: 从噪声语音中提取音频嵌入，通过紧凑编码网络去噪，再用声码器合成清晰语音。

Result: 在语音增强和说话人保真度上优于判别性音频编码器模型，主观测试显示感知质量更优。

Conclusion: 该方法高效且可扩展，性能优于现有技术。
...
### [60] [From Sharpness to Better Generalization for Speech Deepfake Detection](https://arxiv.org/abs/2506.11532)
*Wen Huang,Xuechen Liu,Xin Wang,Junichi Yamagishi,Yanmin Qian*

Main category: eess.AS

TL;DR: 本文研究了语音深度伪造检测（SDD）中的泛化问题，提出锐度作为泛化的理论代理，并通过锐度感知最小化（SAM）提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 语音深度伪造检测中的泛化问题缺乏理论解释，本文旨在通过锐度分析填补这一空白。

Method: 通过分析锐度对领域变化的响应，并应用SAM显式降低锐度。

Result: 实验表明锐度与泛化显著相关，SAM能提升模型在未见测试集上的稳定性能。

Conclusion: 锐度可作为SDD泛化的理论指标，SAM是提升鲁棒性的有效策略。
...
### [61] [Lightweight and Robust Multi-Channel End-to-End Speech Recognition with Spherical Harmonic Transform](https://arxiv.org/abs/2506.11630)
*Xiangzhu Kong,Huang Hao,Zhijian Ou*

Main category: eess.AS

TL;DR: SHTNet是一个轻量级的球谐变换框架，通过几何不变性、空间-频谱注意力融合和随机训练提升多通道语音识别的跨阵列泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决多通道自动语音识别（ASR）中因阵列几何差异导致的跨阵列泛化挑战。

Method: 1. 基于球谐变换的空间声场分解；2. 空间-频谱注意力融合网络（SSAFN）；3. Rand-SHT随机训练增强鲁棒性。

Result: 在异构阵列上平均CER为39.26%，计算量比传统神经波束形成器减少97.1%。

Conclusion: SHTNet通过几何不变性和高效计算，显著提升了跨阵列语音识别的性能。
...
### [62] [Tracking of Spatially Dynamic Room Impulse Responses Along Locally Linearized Trajectories](https://arxiv.org/abs/2506.11703)
*Kathleen MacWilliam,Thomas Dietzen,Toon van Waterschoot*

Main category: eess.AS

TL;DR: 论文提出了一种扩展方法，通过将长轨迹分段为小线性区间，以估计房间脉冲响应（RIRs）的早期部分，适用于更复杂的实际场景。


<details>
  <summary>Details</summary>
Motivation: 测量多空间点的RIRs耗时且模拟需要详细声学环境知识，现有方法仅适用于小区域且未经验证。

Method: 将长轨迹分段为小线性区间，假设每段内直达声和反射声时间间隔不重叠，利用起点和终点的RIRs进行估计。

Result: 使用trajectoRIR数据库验证，方法在复杂房间环境中有效。

Conclusion: 分段扩展方法提高了RIRs估计的适用性和实用性，适用于更复杂的实际场景。
...
### [63] [End-to-End Diarization utilizing Attractor Deep Clustering](https://arxiv.org/abs/2506.11090)
...
### [64] [Assessing the Impact of Anisotropy in Neural Representations of Speech: A Case Study on Keyword Spotting](https://arxiv.org/abs/2506.11096)
...
### [65] [GLAP: General contrastive audio-text pretraining across domains and languages](https://arxiv.org/abs/2506.11350)
...
### [66] [A correlation-permutation approach for speech-music encoders model merging](https://arxiv.org/abs/2506.11403)
...
### [67] [LiLAC: A Lightweight Latent ControlNet for Musical Audio Generation](https://arxiv.org/abs/2506.11476)
...
### [68] [Amplifying Artifacts with Speech Enhancement in Voice Anti-spoofing](https://arxiv.org/abs/2506.11542)
...
### [69] [Dissecting the Segmentation Model of End-to-End Diarization with Vector Clustering](https://arxiv.org/abs/2506.11605)
...
### [70] [(SimPhon Speech Test): A Data-Driven Method for In Silico Design and Validation of a Phonetically Balanced Speech Test](https://arxiv.org/abs/2506.11620)
*Stefan Bleeck*
...
### [71] [Enabling automatic transcription of child-centered audio recordings from real-world environments](https://arxiv.org/abs/2506.11747)
...
### [72] [Abstract Sound Fusion with Unconditioned Inversion Model](https://arxiv.org/abs/2506.11811)
*Jing Liu,EnQi Lian*
...
### [73] [Confidence-Based Self-Training for EMG-to-Speech: Leveraging Synthetic EMG for Robust Modeling](https://arxiv.org/abs/2506.11862)
...
### [74] [Reimagining Dance: Real-time Music Co-creation between Dancers and AI](https://arxiv.org/abs/2506.12008)
...
### [75] [End-to-End Diarization utilizing Attractor Deep Clustering](https://arxiv.org/abs/2506.11090)
*David Palzer,Matthew Maciejewski,Eric Fosler-Lussier*
...
Method: 采用Conformer结构增强说话人表示，引入交叉注意力到吸引子和额外卷积模块；扩展深度聚类，构造标签-吸引子向量以对齐音频嵌入；施加正交约束以改善说话人分离；使用排列不变训练二元交叉熵损失优化说话人检测。

Result: 实验表明，该方法在保持参数数量的同时实现了较低的日志错误率。

Conclusion: 所提框架通过结构化嵌入和高效建模，显著提升了说话人日志的性能。
...
### [76] [Assessing the Impact of Anisotropy in Neural Representations of Speech: A Case Study on Keyword Spotting](https://arxiv.org/abs/2506.11096)
*Guillaume Wisniewski,Séverine Guillaume,Clara Rosina Fernández*
...
<details>
  <summary>Details</summary>
Motivation: 预训练语音表示的各向异性现象普遍存在，但其对下游任务的影响尚不明确，本文旨在探究其在关键词识别任务中的表现。

Method: 使用动态时间规整（Dynamic Time Warping）评估wav2vec2在各向异性情况下的相似性度量效果。

Result: 结果表明，wav2vec2的相似性度量能有效识别单词，且能捕捉语音结构并泛化到不同说话者。
...
### [77] [GLAP: General contrastive audio-text pretraining across domains and languages](https://arxiv.org/abs/2506.11350)
*Heinrich Dinkel,Zhiyong Yan,Tianzi Wang,Yongqing Wang,Xingwei Sun,Yadong Niu,Jizhong Liu,Gang Li,Junbo Zhang,Jian Luan*

Main category: cs.SD

TL;DR: GLAP扩展了CLAP，增加了多语言和多领域能力，在音频-文本检索、语音检索和分类任务中表现优异，并在多语言关键词识别和声音理解方面展现了先进能力。
...
Result: 在多个基准测试中表现优异，尤其在语音检索和分类任务中显著超越现有方法，并在多语言任务中表现突出。

Conclusion: GLAP是一种高效的多语言音频-文本预训练方法，具有广泛的应用潜力。
...
### [78] [A correlation-permutation approach for speech-music encoders model merging](https://arxiv.org/abs/2506.11403)
*Fabian Ritter-Gutierrez,Yi-Cheng Lin,Jeremy H. M Wong,Hung-yi Lee,Eng Siong Chng,Nancy F. Chen*

Main category: cs.SD

TL;DR: 通过相关性置换方法合并独立训练的语音和音乐编码器，显著提升音乐性能。


<details>
  <summary>Details</summary>
Motivation: 统一语音和音乐模型通常需要昂贵的预训练，而模型合并能以较低计算成本实现统一音频模型。

Method: 引入相关性置换方法，逐层对齐音乐和语音编码器的权重空间，计算最大化特征交叉相关性的置换矩阵。

Result: 合并后的模型在保留语音能力的同时，音乐性能显著提升，比线性插值模型平均得分高14.83分。

Conclusion: 该方法可从独立训练的编码器创建统一音频模型。
...
### [79] [LiLAC: A Lightweight Latent ControlNet for Musical Audio Generation](https://arxiv.org/abs/2506.11476)
*Tom Baker,Javier Nistal*

Main category: cs.SD

TL;DR: 提出了一种轻量级模块化架构，显著减少参数数量，同时保持音频质量和条件控制能力。


<details>
  <summary>Details</summary>
Motivation: 现有文本到音频扩散模型缺乏精细的时间变化控制，且ControlNet方法内存占用大、灵活性低。

Method: 采用轻量级模块化设计，减少参数数量，同时匹配ControlNet的音频质量和条件控制能力。

Result: 在客观和主观评估中表现优异，提供更低内存占用和更高灵活性。

Conclusion: 该方法高效且灵活，适用于独立控制的训练和部署。
...
### [80] [Amplifying Artifacts with Speech Enhancement in Voice Anti-spoofing](https://arxiv.org/abs/2506.11542)
*Thanapat Trachu,Thanathai Lertpetchpun,Ekapol Chuangsuwanich*

Main category: cs.SD

TL;DR: 提出了一种模型无关的流程，通过语音增强和噪声处理放大伪造语音中的伪影，显著提升了伪造检测性能。


<details>
  <summary>Details</summary>
Motivation: 伪造语音中通常包含生成模型引入的伪影，但现有方法主要关注架构改进，而忽略了如何增强这些伪影的可见性。

Method: 提出三步流程：噪声添加、噪声提取和噪声放大，结合语音增强模型和噪声处理技术。

Result: 在ASVspoof2019和ASVspoof2021数据集上，伪造检测性能分别提升了44.44%和26.34%。

Conclusion: 该方法通过放大伪造语音中的伪影，显著提升了检测效果，且兼容多种语音增强模型和检测架构。
...
### [81] [Dissecting the Segmentation Model of End-to-End Diarization with Vector Clustering](https://arxiv.org/abs/2506.11605)
*Alexis Plaquet,Naohiro Tawara,Marc Delcroix,Shota Horiguchi,Atsushi Ando,Shoko Araki,Hervé Bredin*

Main category: cs.SD

TL;DR: 论文深入分析了端到端神经说话人日志系统的架构选择，发现微调WavLM编码器和Conformer解码器表现最佳，多类损失优于多标签损失，长块大小提升性能。
...
Method: 研究了不同编码器（SincNet、WavLM）、解码器（LSTM、Mamba、Conformer）、损失函数（多标签、多类）和块大小的影响。

Result: 微调WavLM编码器和Conformer解码器表现最佳，多类损失在多数情况下更优，长块大小提升性能。

Conclusion: 最佳系统在五个数据集上达到SOTA，验证了架构选择的重要性。
...
### [82] [(SimPhon Speech Test): A Data-Driven Method for In Silico Design and Validation of a Phonetically Balanced Speech Test](https://arxiv.org/abs/2506.11620)
*Stefan Bleeck*

Main category: cs.SD

TL;DR: 论文提出了一种名为SimPhon Speech Test-25的新型语音测试方法，通过计算模拟和优化，解决了传统听力测试在评估老年性耳聋时对超阈值缺陷的不足。


<details>
  <summary>Details</summary>
Motivation: 传统听力测试在评估老年性耳聋对语音理解的功能影响时存在不足，尤其是对超阈值缺陷的评估。这促使开发更具诊断特异性的语音感知测试方法。

Method: 采用多阶段计算流程，利用自动语音识别系统模拟听力损失对语音感知的影响，通过声学降级处理识别常见音素混淆模式，并基于语言学语料库筛选优化出25对测试词对。

Result: SimPhon Speech Test-25的诊断性能与标准语音可懂度指数（SII）无显著相关性，表明其能捕捉超出简单可听性的感知缺陷。

Conclusion: SimPhon Speech Test-25为听力测试开发提供了高效的计算优化方法，并已准备好进行初步人体试验。
...
### [83] [Enabling automatic transcription of child-centered audio recordings from real-world environments](https://arxiv.org/abs/2506.11747)
*Daniil Kocharov,Okko Räsänen*
...
<details>
  <summary>Details</summary>
Motivation: 长音频转录对研究儿童语言发展至关重要，但传统手动标注和ASR转录均面临挑战。

Method: 自动检测长音频中可被ASR可靠转录的片段，仅转录这些部分。

Result: 在四个英语长音频语料库中验证，转录13%的语音时，中位WER为0%，平均WER为18%；而全转录时中位WER为52%。
...
### [84] [Abstract Sound Fusion with Unconditioned Inversion Model](https://arxiv.org/abs/2506.11811)
*Jing Liu,EnQi Lian*

Main category: cs.SD

TL;DR: 论文提出了一种声音融合方法，通过反转技术合成新声音，超越简单叠加效果，并基于DPMSolver++采样器设计了SDE和ODE反转模型。


<details>
  <summary>Details</summary>
Motivation: 研究如何生成抽象声音，即不透露真实世界声音事件的声音，同时通过声音融合创造超越简单叠加的新声音。

Method: 采用反转技术，结合DPMSolver++采样器设计SDE和ODE反转模型，消除噪声预测项的循环依赖。

Result: 提出的反转方法无需提示条件，同时在采样过程中保持灵活的引导。

Conclusion: 该方法成功实现了声音融合，生成具有新听觉特征的声音，且无需复杂条件控制。
...
### [85] [Confidence-Based Self-Training for EMG-to-Speech: Leveraging Synthetic EMG for Robust Modeling](https://arxiv.org/abs/2506.11862)
*Xiaodan Chen,Xiaoxue Gao,Mathias Quoy,Alexandre Pitti,Nancy F. Chen*

Main category: cs.SD

TL;DR: 论文提出了一种基于置信度的多说话人自训练方法（CoM2S）和新的Libri-EMG数据集，用于解决语音肌电信号（EMG）到语音转换（V-ETS）中数据不足的问题，实验证明该方法提高了音素准确率并降低了错误率。


<details>
  <summary>Details</summary>
Motivation: 当前V-ETS研究因缺乏配对的EMG-语音数据而受限，阻碍了其发展。

Method: 提出CoM2S方法，利用预训练模型生成的合成EMG数据，并通过音素级置信度过滤机制结合自训练技术优化ETS模型。

Result: 实验表明，该方法提高了音素准确率，减少了音素混淆，并降低了单词错误率。

Conclusion: CoM2S方法有效解决了V-ETS中的数据不足问题，未来将公开代码和Libri-EMG数据集以支持研究。
...
### [86] [Reimagining Dance: Real-time Music Co-creation between Dancers and AI](https://arxiv.org/abs/2506.12008)
*Olga Vechtomova,Jeff Bos*

Main category: cs.SD

TL;DR: 论文提出了一种双向互动系统，让舞者通过动作动态塑造音乐环境，实现舞者与AI的创造性合作。


<details>
  <summary>Details</summary>
Motivation: 传统舞蹈表演中，动作是对音乐的被动响应，而AI在舞蹈中的应用多限于从音乐生成编舞。本文旨在打破这种单向关系，探索舞者如何通过动作主动影响音乐。

Method: 采用多模态架构，通过智能组合预录音乐片段，实时响应舞者动作，形成连贯音乐作品。

Result: 通过表演数据分析，揭示了动作特征与音频特征之间的新兴沟通模式。

Conclusion: 该系统重新定义了AI在表演艺术中的角色，使其成为响应式合作者，为专业舞蹈和即兴艺术表达提供了新可能。

...
Motivation: 在高移动性场景中，准确估计和校正TO和CFO对多用户OTFS系统的性能至关重要，尤其是TO估计用于定位用户导频，CFO估计用于提高信道估计精度。

Method: 1. 提出基于SU-PCP导频结构的TO估计技术；2. 提出MU-PCP导频模式，利用滤波器组分离用户信号并估计TO；3. 提出基于Chebyshev多项式的CFO估计技术，将多维ML搜索问题简化为多个一维搜索问题。

Result: 通过改进导频结构和估计技术，实现了更准确的TO和CFO估计，提高了系统性能。

Conclusion: 所提出的同步技术在高移动性场景下有效提升了多用户OTFS系统的性能，为实际应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 由于电子可重构天线在分析建模上的挑战，以及流体天线系统（FAS）日益增长的关注，本文旨在提供一个实用的解析模型。

Method: 采用动态超表面天线（DMA）实现FAS，利用电路理论重新构建信号模型，并通过全波仿真验证。

Result: 模型验证显示与仿真结果一致，且DMA实现的FAS性能接近理想化的位置灵活天线。

Conclusion: 提出的解析模型为FAS系统设计提供了理论支持，并展示了DMA实现的FAS的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何利用STAR-RIS技术提升ISAC网络的性能，解决感知与通信的联合优化问题，同时满足严格的感知、安全和硬件约束。

Method: 采用交替优化（AO）框架，将问题分解为子问题，分别通过KKT条件和SCA方法优化基站波束成形和STAR-RIS参数。

Result: 仿真表明，所提算法在通信总速率上显著优于被动RIS和单STAR-RIS基线，同时满足感知和安全约束。

Conclusion: 主动STAR-RIS在ISAC网络中具有显著性能优势，联合优化方法有效解决了非凸优化问题。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏对DPE理论中多径误差的理论描述，研究旨在填补这一空白。

Method: 通过几何分析量化多径误差对CAF和PVT解的影响，提出SCMB模型，并通过蒙特卡洛模拟和城市峡谷测试验证。

Result: 最大PVT偏置取决于多径误差的最大值，且PVT偏置随卫星仰角增加而增大。
...
<details>
  <summary>Details</summary>
Motivation: 由于IM-DD系统的独特模型和内存效应，概率整形技术的有效应用仍是一个开放性问题。

Method: 采用动态选择性映射（DSLM）机制控制信号包络，并结合修改的M-BCJR算法的turbo均衡器。

Result: 在56GBaud PAM8系统中，接收灵敏度提升了1dB，且兼容典型概率幅度整形架构。

Conclusion: 该方案为内存效应下的PPC IM-DD系统应用概率整形技术提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: GNSS信号易受干扰和欺骗攻击，现有方法需要先验知识，SCHIEBER旨在解决这一问题。
...
Method: ICWLM直接从大规模混合无线数据训练，利用上下文学习（ICL）适应不同系统配置，并采用动态权重平均（DWA）算法平衡多任务损失。

Result: ICWLM在多项物理层任务中表现优异，泛化能力强，适用于未见过的系统配置。

Conclusion: ICWLM为未来无线网络提供了统一且自适应的AI模型范式，降低了部署复杂性并提升了资源管理智能性。
...
<details>
  <summary>Details</summary>
Motivation: 低分辨率、非线性或宽带量化器的性能优化需求，尤其是在存在高功率带外干扰时，需要高效的校正方法。

Method: 结合参数化模型（输入信号、噪声和干扰信号的预期）优化LUTs，提出三种分析估计器，并针对特定信号类型（如PSK和LFM）提出近似方法以提高计算可行性。

Result: 模拟结果显示，该方法能高精度实时恢复输入信号，抑制谐波失真，并在3位量化器和12抽头模型下实现MSE改善>10 dB和SFDR改善>20 dBc。

Conclusion: 所提出的估计器在性能上显著优于传统方法，且对输入参数变化、非线性量化器和时变干扰源具有鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模MIMO系统中深度学习模型训练所需高质量本地数据难以获取的问题，同时降低能耗并动态适应用户需求。

Method: 使用Transformer基础模型进行预编码，引入数据增强方法通过预训练特征提取器找到与目标分布相似的训练样本。

Result: 在相同能耗下，零样本部署的基础模型显著优于零强制方法，接近加权最小均方误差性能，且复杂度降低8倍。

Conclusion: 该工作通过解决数据可用性和训练复杂度问题，实现了基于深度学习的解决方案在实际中的应用，并支持动态配置用户速率需求以提高能效和公平性。
...
<details>
  <summary>Details</summary>
Motivation: 传统说话者分类（SD）仅提供通用标签（如说话者1、说话者2），而角色分类（如医生与患者、主持人与嘉宾）更具应用价值。

Method: 1. 使用强制对齐和交叉熵损失简化训练；2. 为词预测和角色预测设计独立的预测器；3. 利用RD后验活动优化ASR解码。

Result: 模型在角色分类任务中表现更优，同时减少了ASR解码中的小词删除错误。

Conclusion: 分离任务预测器和利用RD后验活动能显著提升联合ASR+RD系统的性能。
...
<details>
  <summary>Details</summary>
Motivation: 语音障碍影响广泛，自动非侵入性诊断技术能显著改善医疗质量和患者生活。传统DNN虽有效但决策过程不透明，限制了临床信任。

Method: 研究采用可解释AI（XAI）方法，特别是概念瓶颈模型（CBM）和概念嵌入模型（CEM），以提高模型透明度和解释性。

Result: 概念模型（如CBM和CEM）在性能上可与传统深度学习方法媲美，同时提供更透明的决策框架。

Conclusion: 可解释AI方法（如CBM和CEM）在语音障碍诊断中具有潜力，既能保持高性能，又能增强临床信任。
...
<details>
  <summary>Details</summary>
Motivation: 推动多通道、可泛化的联合ASR和说话人日志技术的研究，分析当前技术的局限性和发展趋势。

Method: 通过挑战赛设计、评估指标、数据集和基线系统，分析参与者提交的系统及其趋势。

Result: 端到端ASR系统成为主流，引导源分离仍占主导，说话人日志优化是关键。尽管技术进步，复杂环境下的语音识别仍困难。

Conclusion: CHiME挑战赛展示了端到端ASR的优势和当前技术的局限性，强调了说话人日志和复杂环境下的进一步研究需求。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM在ASR中表现出高准确性和多方言支持，但其高解码延迟难以满足实时需求。现有推测解码方法未充分考虑ASR任务特性，速度提升有限。

Method: SpecASR通过自适应草稿序列生成、草稿序列重用策略和两阶段稀疏令牌树生成算法，优化解码效率。

Result: 实验表明，SpecASR在保持识别准确性的同时，比基线自回归解码和推测解码分别提速3.04x-3.79x和1.25x-1.84x。

Conclusion: SpecASR显著降低了ASR的实时延迟，为LLM在实时语音识别中的应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统语音增强方法在高混响环境中的性能不足问题。

Method: 结合双路径MCLP滤波器（时间和频率维度）和多范数波束形成，最小化麦克风阵列输出功率和去噪信号的l1范数，同时保留目标方向信号。

Result: 在评估中，该方法在强混响场景下显著优于基线方法。

Conclusion: 提出的方法在语音增强中具有优越性能，尤其适用于高混响环境。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统说话人日志框架在实时多说话人跟踪中的低延迟和高效性问题。

Method: 使用AOSC按到达时间顺序存储说话人嵌入，动态更新缓存以优化存储和跟踪精度。

Result: 在基准数据集上验证了方法的有效性和灵活性，适用于低延迟场景。

Conclusion: Streaming Sortformer为实时多说话人跟踪提供了稳健解决方案，并为流式多说话人语音处理奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 解决语音预训练模型中音色与内容信息难以解耦的问题，避免音色信息泄露并提升内容处理任务性能。

Method: 提出InterpTRQE-SptME基准和InterpTF-SptME方法，利用SHAP技术量化音色残留并过滤音色信息。

Result: 实验表明SHAP Noise方法可将音色残留从18.05%降至接近0%，同时保持内容完整性。
...
TL;DR: 论文研究了歌词到歌曲生成模型（LS2）对训练数据记忆的脆弱性，提出了一种名为APT的攻击方法，揭示了模型在音频和视觉领域的记忆问题。


<details>
  <summary>Details</summary>
Motivation: 探索歌词到歌曲生成模型对训练数据记忆的脆弱性，尤其是在语义被修改但声学结构保留的情况下。

Method: 使用APT攻击方法，通过同音替换修改歌词，测试模型在音频和视觉领域的记忆行为。

Result: 发现模型在音频领域（如SUNO和YuE）和视觉领域（如Veo 3）均表现出对训练内容的记忆，触发相似输出。
...
<details>
  <summary>Details</summary>
Motivation: SELD在机器认知任务中具有重要价值，如环境推断和导航。今年的挑战新增了距离估计，以更全面评估模型。

Method: 使用log-mel频谱图和强度向量，结合多种数据增强技术，提出基于EINV2的网络架构。

Result: 在开发数据集测试集上，F-score为40.2%，DOA误差17.7度，RDE误差0.32。

Conclusion: 提出的方法在音频事件定位与检测任务中表现良好，为后续研究提供了参考。
...
Method: 任务I：集成语言识别和多语言MOE LoRA结构，使用CTC预测标记作为提示；任务II：替换基线模型为更适合的英语版本。
...
Method: 结合冻结扩散模型与双适配器架构，分两阶段训练：ASR目标对齐语义表示，再通过合成音频-文本对学习指令跟随。

...
Motivation: 临床需要综合分析多种生理信号来监测血流动力学，但现有研究多集中于单信号分析，缺乏适用于真实临床场景的复杂信号分析方法

Method: 基于自监督学习的掩码表示学习方法，通过恢复三种被掩码的生理信号（ECG、PPG、ABP）来提取反映心脏周期电学、压力和流体特性的生理特征

Result: 在低血压、每搏输出量、收缩压、舒张压和年龄预测等临床下游任务中表现优异，特别是在使用非侵入性信号的预测任务中显著超越其他模型

Conclusion: SNUPHY-M是首个将多模态自监督学习应用于心血管分析（涉及ECG、PPG和ABP信号）的模型，有效支持临床决策和精准诊断，为非侵入性血流动力学早期诊断和管理做出重要贡献
...
<details>
  <summary>Details</summary>
Motivation: AI技术虽然取得重大突破，但如何公平负责任地使用AI来改善全球人类状况仍面临挑战，特别是在教育领域的应用需要解决技术和社会问题。

Method: 提供关于AI在教育环境中使用的核心技术问题入门，包括确保公平包容性、处理幻觉输出、资源高效使用等，并通过开发沉浸式结构化"智能教科书"进行说明。

Result: 文章为研究者和教育者提供了推进AI在工程教育中作用的资源，展示了如何将AI工具实际应用于改善教育体验。

Conclusion: AI在教育领域，特别是信号处理教育中具有巨大潜力，但需要解决公平性、透明度、可解释性和可信性等技术和社会挑战，才能实现其真正价值。
...
<details>
  <summary>Details</summary>
Motivation: CBCT扫描中的散射伪影严重降低图像质量，虽然基于深度学习的方法在估计散射方面显示出潜力，但由于网络内存占用大，在移动CBCT系统或边缘设备上的部署仍然受限。

Method: 首先在六个分辨率下检查CBCT散射信号的下采样-上采样重建误差，比较四种插值方法；然后在五个图像分辨率上训练最新的最先进方法，评估FLOPs、推理时间和GPU内存需求的减少。

Result: 减少输入大小和网络参数实现了与基线方法相比78倍的FLOPs减少，同时保持可比较的性能（MAPE从4.42%降至3.85%，MSE从2.01×10⁻²降至1.34×10⁻²），推理时间和GPU内存使用分别减少了16倍和12倍。

Conclusion: 本研究强调了在基于深度学习的散射估计中下采样的被低估作用，通过我们的方法实现的FLOPs和GPU内存需求的大幅减少，使得在资源受限环境（如移动CBCT和边缘设备）中进行散射校正成为可能。
...
<details>
  <summary>Details</summary>
Motivation: 6G网络需要在长期复杂中断环境中保持稳健运行，从效率优先转向稳健性、可持续性和效率并重的设计理念，以应对移动网络与能源、交通、应急服务等关键基础设施之间的依赖关系和聚变故障风险。

Method: 提出3R框架（Reliability、Robustness、Resilience）来形式化稳健性，转换为可测量能力：渐进性能降级、情境感知、快速重配置、学习驱动改进和恢复。采用边缘本地化设计、开放接口、程序化能力支持岛屿化运行和备份模式，通过AI原生控制循环、硬件根源的零信任安全、优先级流量管理等技术实现。

Result: 识别了九个业务模式组和多个与3R目标一致的模式，提出了开放平台和高质量补充服务能够产生生态系统外部性效应，同时开拓新市场。还给出了相关的管理和标准化建议。

Conclusion: 本白皮书作为6G稳健性研究的初步之作，为研究人员、专业人士、政府官员和公众提供了理解和形成6G稳健性发展的核心组成要素，期待引发更广泛的研究和应用。
...
<details>
  <summary>Details</summary>
Motivation: 睡眠预测可以帮助个人和医疗提供商预先预测并主动处理影响睡眠质量的因素，最终改善心理和身体健康。

Method: 提出AdaST-Sleep模型，结合卷积层捕捉多特征间的空间相互作用，通过递归网络层处理长期健康数据，并集成域分类器来实现不同主体间的演续性。

Result: 在5种输入窗口大小(3,5,7,9,11天)和5种预测窗口大小(1,3,5,7,9天)的实验中，该方法均超过4种基线模型，最低RMSE为0.282(7天输入窗口+1天预测窗口)，甚至在多天预测中也保持良好性能。

Conclusion: 该框架为使用商业可穿戴设备的稀疏数据和域适配技术进行个性化睡眠预测提供了突出的稳健性和适应性。
...
<details>
  <summary>Details</summary>
Motivation: 因为FORCES成像方案具有固定的移动方向焦点和大的发射孔径，在距移动焦点远处时对焦性能力差，需要改进该方案的移动方向对焦性能力。

Method: 对FORCES和uFORCES方法应用回顾性发射波束成形(RTB)技术，在移动方向实现全平面的发射对焦。通过纳米线幻象和囊性幻象实验评估性能。

Result: 实验结果显示，应用RTB后FORCES和uFORCES在距移动焦点远处的对焦性能力都得到显著提升。在焦点处，性能保持相当或有所改善。通过半高宽度和对比度噪声比指标进行了量化验证。

Conclusion: 回顾性发射波束成形技术能够有效改善TOBE阵列FORCES成像方案的移动方向对焦性能，在保持原有优势的同时扩大了良好对焦的区域范围。
...
<details>
  <summary>Details</summary>
Motivation: 现有符号图学习方法通常假设所有节点都可用，但在实际应用中往往只有部分节点可观测，其余节点保持隐藏状态，需要解决这种部分观测场景下的符号图识别问题

Method: 基于符号图上信号平滑的假设，将符号图拓扑推断构建为带列稀疏正则化的约束优化问题，使用定制的块坐标下降(BCD)方法求解

Result: 在合成数据和真实数据上的实验结果表明所提出的SGL-HNCS方法具有高效性

Conclusion: 该方法能够有效处理存在隐藏节点的符号图学习问题，通过列稀疏正则化和BCD优化算法实现了良好的性能
...
<details>
  <summary>Details</summary>
Motivation: 解决数据序列的非参数聚类问题，其中每个序列包含从未知分布生成的i.i.d.样本，目标是在给定错误概率下通过观察最少样本量来估计聚类。

Method: 提出SEQ-SPEC算法和计算效率更高的IA-SEQ-SPEC算法，前者确保有限时间停止和指数一致性，后者在保持性能的同时提高计算效率。

Result: 仿真实验表明，两种算法均优于固定样本量SPEC、SEQ-KMED和SEQ-SLINK算法，IA-SEQ-SPEC在计算高效的同时性能接近SEQ-SPEC。

Conclusion: 这是首个在序列框架下进行数据序列谱聚类的研究工作，提出的算法在合成和真实数据集上均表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统滤波方法需要完整的图信号先验信息，要么通过网格搜索确定参数，要么使用梯度下降优化，限制了实际应用。

Method: 将联合时-顶点分数阶傅里叶变换域的维纳滤波模型嵌入神经网络框架，采用数据-模型协同驱动的方式，通过数据驱动方法更新变换阶数对和滤波器系数。

Result: 实验表明JFRFFNet在输出信噪比方面相比现有先进方法有显著提升。

Conclusion: JFRFFNet方法仅需部分先验信息就能实现有效的时变图信号去噪，为图信号处理提供了更实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统采样方法独立处理每个通道导致数据冗余，多通道随机信号存在统计相关性，需要利用这种相关性实现高效采样。

Method: 将相关通道建模为少量不相关宽平稳潜在源的线性组合，通过潜在源的频谱分割、时空采样和插值构建多波段采样方案。

Result: 实验证明该方法在理论采样密度下实现了近乎无损的重建，验证了其效率。

Conclusion: 提出的多波段采样方案达到了理论最小采样密度，为相关多通道信号的高效采样提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: EEG信号清洗是研究领域的关键挑战，现有方法依赖人工超参数调整、对异常值敏感且计算成本高，需要自动化解决方案来替代耗时的主观人工检查。

Method: 提出了改进的黎曼土豆田（iRPF）方法，这是一种快速、全自动的EEG伪影去除技术，解决了当前方法的主要局限性。

Result: 在226个EEG记录上测试，iRPF在召回率、特异性、精确度和F1分数上分别获得高达22%、102%、54%和24%的提升，处理速度达到每epoch 8毫秒以内。

Conclusion: iRPF为脑机接口和临床神经影像应用提供了强大、数据驱动的伪影去除解决方案，适用于高质量EEG预处理。
...
<details>
  <summary>Details</summary>
Motivation: 为了在不同结构之间建立一个共享的分析基础，使得可以使用一个结构的特征模态来分析另一个结构的散射特性，从而提高设计效率和可比性。

Method: 推导了一种形式化方法，定义了模态变换矩阵来描述两个结构之间特征场和权重系数的映射关系，并实现不同基底下扰动矩阵的转换。

Result: 证明在这种情况下散射矩阵和扰动矩阵不再是对角矩阵。通过两个例子验证和展示了该方法的有效性，特别是在天线元件渐进式修改设计过程中的应用。

Conclusion: 该形式化方法为使用共享特征模态基础分析不同结构的散射特性提供了理论支撑，在天线设计和结构优化中具有应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 像素流体天线虽然提供更好的复用增益和快速波束切换能力，但引入了状态不可分离信道响应的新挑战，需要专门的信道估计和预编码方案。

Method: 使用近似可分离信道响应模型和DNN天线辐射函数构建稀疏信道恢复框架，提出基于正交匹配追踪和变分贝叶斯推理的低复杂度信道估计算法，并设计模拟预编码方案优化天线切换状态选择。

Result: 仿真结果表明，所提方法在多个基线方法中表现最优，特别是在高信噪比和多用户场景下性能提升显著。

Conclusion: 该研究为像素流体天线系统提供了有效的信道估计和预编码解决方案，解决了状态不可分离信道响应问题，展现了在实际系统中的良好应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统深度学习方法在飞行器半泥米波网络中的通用性不足、对噪声输入的稳健性以及对飞行器高度变化的敏感性问题。

Method: 设计UNet结构模型，结合多尺度特征提取、卷积特征融合和ASPP瓶颈结构，从对数距离、视线掩码和建筑掩码输入预测路径损耗地图，同时开发了全向量化视线掩码计算算法。

Result: 在自建光线追踪数据和RadioMapSeer案例上评估，模型在准确性和效率方面都超过多个现有最优方法，并公开了源代码。

Conclusion: 该方法为飞行器半泥米波网络提供了高效准确的路径损耗预测方案，具有良好的通用性和可复现性，有助于未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 汽车座舱声学环境复杂，传统方法在音质和空间定位精度之间存在权衡，需要一种能够同时改善两者的解决方案

Method: 提出空间功率图(SPM)约束来表征再现声场的角度能量分布，使用波束成形技术引导能量朝向目标方向；将SPM约束集成到多通道均衡框架中，并采用深度学习优化神经网络来解决非凸优化问题

Result: 通过现场客观和主观评估证实，该方法在汽车座舱内显著提升了音质和空间定位能力，并分析了不同音频材料和虚拟声源到达角度的影响因素

Conclusion: SPMnet方法成功解决了复杂声学环境中音质与空间定位的权衡问题，为汽车音频系统提供了高质量的声场再现解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有的源分离系统客观评估方法与主观人类感知存在差异，特别是当漏泰和自身失真相互作用时

Method: 使用颇演映射技术将信号编码投影到流形多形上，通过马哈拉比斯距离测量自身失真（PM）和漏泰（PS）

Result: 在英语、西班牙语和音乐混合信号上，PS和PM与人类平均意见分数的线性相关系数达到86-87%，超过14个竞争方法

Conclusion: 这两个可微分的指标能够在系统性能下降时互补信息，提供了更可靠和信息丰富的评估方法
...
<details>
  <summary>Details</summary>
Motivation: 现有的ASV系统容易受到线上和无线对抗攻击，但缺乏全面的数据集来测试防御方法，需要开发更强大的数据集和防御机制。

Method: 开发AdvSV 2.0数据集，包含经典对抗攻击算法和ASV系统；提出基于神经重放模拟器(NRS)的新攻击方法；设计CODA-OCC对比学习单分类防御框架。

Result: CODA-OCC在AdvSV 2.0数据集上达到11.2%的EER和0.95的AUC，优于现有检测方法。

Conclusion: AdvSV 2.0数据集填补了ASV对抗攻击测试的空白，NRS攻击方法增强了OTA攻击威胁，CODA-OCC防御方法有效提升了检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 虽然使用语音语言线索进行图像检索在多模态感知中展现出突出潜力，但在多讲话人场景中利用语音仍面临挑战。需要解决如何在存在多个讲话人的情况下，准确提取目标讲话人的语音命令并与对应图像对齐的问题。

Method: 提出了一种框架，通过目标讲话人感知对比学习方法，将预训练的自监督音频编码器与视觉模型集成。该方法基于目标讲话人提取和检索模块，能够在存在多个讲话人的情况下提取目标讲话人的语音命令并与对应图像进行对齐。

Result: 在SpokenCOCO2Mix和SpokenCOCO3Mix数据集上的实验表明，TSRE方法显著超过现有方法，在2个讲话人场景中达到36.3%的Recall@1，在3个讲话人场景中达到29.9%的Recall@1。这是对单讲话人基线模型和最新模型的显著收益。

Conclusion: 该方法在多讲话人语音-图像检索任务中表现出艰强的性能，显示了在辅助机器人和多模态交互系统中实际部署的潜力，为多讲话人多模态感知领域提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究空间选择性注意力在鸡尾酒会情境中的重要性，以及短期认知疲劳是否会影响这种注意力，特别是在生态化设置中的费力对话后。

Method: 年轻正常听力参与者在三种条件下（嘈杂环境对话、被动听噪声、安静环境对话）前后执行听觉空间选择性注意任务，测量自我报告的努力感和疲劳度以及任务表现。

Result: 嘈杂环境对话和被动听噪声后自我报告的疲劳度增加，但注意任务反应时间反而减少，准确率无系统性变化，且存在强烈的训练效应。

Conclusion: 与预期相反，嘈杂环境中的对话并未损害空间选择性注意力，反而可能通过某种机制改善了反应时间，同时训练效应显著影响实验结果。
...
<details>
  <summary>Details</summary>
Motivation: 传统鼻音评估方法（如鼻咽镜和鼻测量法）对儿童不友好且体验不佳，需要开发非侵入性的替代方案。

Method: 使用语音反演技术，整合电声门图源信息和声学特征（F0、周期性/非周期性能量）作为声门控制代理，先在成人数据上训练，然后在儿童VPI数据上微调。

Result: 相比之前的SI系统，在鼻音估计上获得16.92%的皮尔逊相关系数相对提升；经过儿童数据微调后，相比微调前性能提升7.90%

Conclusion: 语音反演结合声门控制信息和跨年龄组微调是有效的非侵入性鼻音评估方法，特别适用于儿童VPI患者的临床评估。
...
<details>
  <summary>Details</summary>
Motivation: 传统音频标签任务无法标签空间音频中特定区域（如角度空间或距离）的声音事件，需要新方法来解决这个问题。

Method: 研究了不同谱、空间咄位置特征的组合效果，并将PANNs咄AST等现有音频标签系统扩展到区域特定标签任务。

Result: 在模拟咄实际数据集上的实验结果显示了该任务的可行性咄方法的有效性，同时发现统合方向特征对全向标签有益。

Conclusion: 区域特定音频标签是一个可行的新任务，通过结合空间特征咄现有深度学习模型可以有效地实现该任务。
...
<details>
  <summary>Details</summary>
Motivation: 传统声学边界条件测量方法依赖简化假设，限制了在真实场景中的有效性，需要开发更精确的原位阻抗估计方法。

Method: 采用基于模拟推理的贝叶斯框架，利用神经网络架构直接将模拟数据映射到模型参数的后验分布，使用带分数阶微积分项的阻尼振荡器模型建模阻抗行为。

Result: 在长方体房间有限元模型和阻抗管测量中验证了方法的有效性，成功估计所有六个独立阻抗，在汽车舱室模型中展示了可靠的uncertainty量化和高预测精度。

Conclusion: 该方法具有通用性、高效性和物理一致性，为真实室内环境的声学边界条件表征提供了有前景的解决方案。
...
Method: 结合低秩适应(LoRA)和专家混合路由器的参数高效框架，只微调选定的专家，支持领域自适应

Result: 在ASVSpoof 5评估集上达到5.56%的等错误率(SOTA)，无需数据增强，路由器能根据攻击类型智能选择专家

Conclusion: MoLEx框架有效降低了计算成本，保持了强大性能，并展现出对新型伪造攻击的领域感知适应能力
...
<details>
  <summary>Details</summary>
Motivation: 现实世界音频通常包含混合的语音和背景声音，下游任务需要选择性访问这些组件。现有通用音频编解码器学习纠缠表示，而特定编解码器虽然提供解耦表示但仅限于语音处理。
...
Result: 实验结果显示DeCodec在保持先进信号重建的同时，实现了新功能：通过表示重组在嘈杂语音上实现优越的语音增强和有效的一次性语音转换，通过干净的语义特征提高ASR鲁棒性，以及在TTS中实现可控的背景声音保留/抑制。

Conclusion: DeCodec作为一个通用的解耦表示学习器，通过分层解耦实现灵活的特征选择，使其成为多个音频应用的通用前端。
...
<details>
  <summary>Details</summary>
Motivation: 传统音频深度伪造检测评估方法存在两个主要问题：1）使用单一EER指标会因合成器样本数量不均而权重失衡；2）真实语音数据缺乏多样性（通常只有单一环境和语音风格），无法模拟真实场景。

Method: 提出bona fide cross-testing评估框架，整合多种真实语音数据集，通过聚合多个EER来实现更平衡的评估。对150多个合成器在9种真实语音类型上进行基准测试，并发布新数据集。

Result: 新评估框架相比传统方法在鲁棒性和可解释性方面有所提升，提供了更全面的性能评估。

Conclusion: bona fide cross-testing框架为音频深度伪造检测提供了更可靠和全面的评估标准，有助于推动该领域的研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决音频场景分类中的双重挑战：严格的复杂度约束和对已知及未知设备的鲁棒泛化，同时利用测试时设备标签的新规则

Method: 使用CP-MobileNet学生模型从双教师集成（标准PaSST教师和设备感知特征对齐专家）学习知识蒸馏，最后进行设备特定微调

Result: 在开发集上达到57.93%的最终准确率，相比官方基线有显著提升，特别是在未知设备上表现优异

Conclusion: 提出的知识蒸馏框架结合设备感知特征对齐和测试时设备标签利用，有效实现了低复杂度下的设备鲁棒音频场景分类
...
<details>
  <summary>Details</summary>
Motivation: 传统Transformer模型因自注意力机制的二次复杂度，无法一次性处理整个音乐作品，需要滑动窗口方式处理，影响效率和长期依赖捕捉能力。

Method: 提出了一种高效稀疏注意力架构：1)编码器和解码器都使用滑动窗口自注意力；2)混合全局-局部交叉注意力机制，根据MIDI标注类型关注不同范围；3)在编码器和解码器之间使用层次池化策略降低计算负荽。

Result: 在MAESTRO数据集上进行实验，证明提出的模型实现了显著的计算成本和内存使用减少，加快了推理速度，同时保持了与全注意力基线模型相当的转译性能。

Conclusion: 稀疏注意力机制可以用于构建高效且高性能的链琴转译系统，允许在同样硬件上训练更长的音频上下文，为实际应用提供了可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经音频编解码器主要依赖RVQ，而FSQ作为新兴替代方案具有训练简单和单码本支持的优势，但需要验证其在噪声环境下的鲁棒性。

Method: 提出基于FSQ的NeuCodec编解码器，通过编码器蒸馏实验验证不同编码器可产生不同但等价的编码序列，并比较RVQ和FSQ在模拟噪声信道传输中的性能。

Result: FSQ编码具有内置冗余特性，在噪声信道中表现出远超RVQ的比特级扰动鲁棒性，不同编码器可学习到不同但重建质量相当的编码序列。

Conclusion: FSQ为基础的神经音频编解码器在噪声鲁棒性方面具有显著优势，为实际通信应用提供了更可靠的音频编码解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有零样本文本转语音方法存在的推理速度慢和重复伪影问题，充分利用离散表示的优势，避免将离散token嵌入连续空间的做法

Method: 采用纯离散流匹配方法，在紧凑统一架构中显式建模分解的语音属性，利用上下文学习条件化文本内容和参考语音提取的韵律声学属性，使用分解流预测机制

...
Motivation: 现有信道模型缺乏对ISAC特定动态的全面表征，特别是单静态和双静态感知配置之间的关系，需要实证测量来弥补这一空白。

Method: 使用79-GHz FMCW信道测量仪在动态城市微小区环境中进行实证测量，覆盖7个真实场景和移动目标/收发器。
...
TL;DR: 提出了一种基于提示学习的智能反射面相位信息压缩系统，通过混合提示技术和可变速率方法，在单一模型中实现自适应压缩，显著降低重构误差。


<details>
  <summary>Details</summary>
Motivation: 智能反射面技术面临控制链路带宽限制导致的频繁相位信息传输开销问题，现有深度学习方法存在解码器复杂度高、适应性差和压缩比固定等局限。

Method: 采用提示学习与特征线性调制结合的混合提示技术，通过潜在掩码实现可变压缩比，并设计轻量级深度卷积门控解码器。

Result: 相比传统自编码器基线显著降低归一化均方误差，在各种信道条件下保持鲁棒性，单一模型支持多种压缩比。
...
TL;DR: 提出了一种基于Hyvarinen分数的min-SCUSUM方法，用于多流快速变化检测和故障隔离，无需显式分布即可计算分数函数差异替代似然比。


<details>
  <summary>Details</summary>
Motivation: 传统最优算法需要显式的前后变化分布来计算观测值的似然比，对于高维数据和复杂机器学习模型计算成本高甚至不可行。

Method: 使用Hyvarinen分数计算分数函数差异替代对数似然比，提出min-SCUSUM方法进行多流快速变化检测和故障隔离。

Result: 算法渐近性能取决于前后变化分布之间的Fisher散度，并建立了故障误识别概率的上界。

Conclusion: min-SCUSUM方法为复杂模型和高维数据提供了一种计算高效的快速变化检测和故障隔离解决方案。
...
TL;DR: 本文研究了可移动天线辅助的低地球轨道卫星集成感知与通信系统，通过联合优化波束成形和天线位置，在满足通信SINR约束下最小化感知性能误差。


<details>
  <summary>Details</summary>
Motivation: 低地球轨道卫星的严重信号衰减和有限发射功率会降低ISAC性能，需要解决这一性能退化问题。

Method: 采用半定松弛简化复杂问题，提出基于交替优化的算法将原问题分解为两个子问题并分别求解。

Result: 仿真验证了算法的收敛性和有效性，相比基准方法实现了通信与感知性能的更好权衡，感知性能提升至少25%。

Conclusion: 可移动天线辅助的LEO ISAC系统能有效提升系统性能，在通信和感知之间实现更好的权衡。
...
TL;DR: 本文提出了一种从粗到细的递归方法，用于在用户位置随机且仅知用户密度分布和障碍物配置的情况下，优化可重构智能表面(RIS)的部署位置，以最大化系统的期望最小信干噪比。


<details>
  <summary>Details</summary>
Motivation: 现有RIS部署研究大多假设用户位置已知，但实际部署前通常只知道用户密度分布和障碍物配置，因此需要开发能在用户随机性下优化RIS位置的方法。

Method: 提出递归从粗到细的方法：基于障碍物配置构建候选位置集，通过多次用户分布实例评估，并在每阶段识别的最优区域内递归细化搜索，最终确定RIS部署的最优区域。

Result: 数值结果验证了所提方法的有效性，能够在用户位置随机的情况下找到RIS的最优部署位置。

Conclusion: 该方法解决了实际部署中用户位置未知的问题，通过递归搜索策略有效优化了RIS的部署位置，提升了系统性能。
...
TL;DR: 本文综述了基于噪声的通信技术，该技术通过将信息编码到噪声的统计特性中，而非使用传统确定性载波，为6G网络和大规模物联网提供超低功耗、安全和隐蔽的无线通信解决方案。


<details>
  <summary>Details</summary>
Motivation: 第六代（6G）网络和大规模物联网（IoT）的普及需要超低功耗、安全和隐蔽的无线通信技术，基于噪声的通信作为一种变革性范式应运而生。

Method: 系统探索了基于噪声通信的基本原理和关键方法，包括热噪声调制（TherMod）、噪声调制（NoiseMod）及其变体，以及基尔霍夫定律-约翰逊噪声（KLJN）安全密钥交换。

Result: 分析证实基于噪声的系统在能源效率和隐蔽性方面具有无与伦比的优势，并解决了信道估计和硬件实现等关键实际挑战。
...
Conclusion: MCAKD框架使实时基础模型部署在边缘设备上成为可能，解决了大规模模型部署的瓶颈问题。
...
TL;DR: 本文研究了AFDM波形在脉冲整形下的模糊函数特性，发现AFDM可以通过调整参数c₁灵活控制旁瓣凹陷位置，并提出了一种新的参数设计方法来减轻强目标对弱目标检测的影响。


<details>
  <summary>Details</summary>
Motivation: 研究AFDM波形在ISAC系统中的模糊函数特性，探索如何通过波形设计改善弱目标检测性能。

Method: 推导了无脉冲整形AFDM波形的平均平方离散周期模糊函数闭式表达式，并比较了AFDM、OFDM和OCDM波形的模糊函数特性。

Result: AFDM波形可以通过调整参数c₁灵活控制旁瓣凹陷位置，脉冲整形滤波器能够沿延迟轴形成成形主瓣，沿多普勒轴形成快速滚降旁瓣。

Conclusion: AFDM波形在ISAC系统中具有灵活的参数调控能力，可以有效改善弱目标检测性能，理论分析和设计方法得到了数值验证。
...
TL;DR: 提出了BTTDA方法，一种基于张量的监督特征提取方法，通过灵活的多元降维提高脑电信号分类准确率，在ERP解码中达到91.25%的ROC-AUC性能。


<details>
  <summary>Details</summary>
Motivation: 现有张量方法如Tucker和PARAFAC分解缺乏灵活性，无法充分捕捉脑电数据的复杂性，需要更有效的特征提取方法来提升脑机接口性能。

Method: BTTDA扩展了高阶判别分析(HODA)，结合可解释的前向模型和紧缩方案，迭代提取判别块项，改进分类特征表示。

Result: 在MOABB基准测试中，BTTDA和PARAFACDA在ERP解码上显著优于传统HODA方法，达到91.25%的ROC-AUC；在运动想象任务中，BTTDA也显著优于HODA(64.52% > 61.00%)。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态动作识别在传感器失效或噪声时性能急剧下降的问题，以及依赖大量标注数据的问题。

Method: 联合优化三个目标：跨模态对比对齐、模态内自蒸馏提高视图不变性、退化模拟训练模型从损坏输入中恢复；推理时使用自适应模态门控网络进行鲁棒融合。

Result: 在NTU RGB+D 120和UWA3D-II数据集上达到SOTA准确率，在严重双模态丢失时仅下降11.5%，显著优于强监督融合基线。
...
Result: 加入模型反演的InSAR特征或InSAR相干性层后，反演精度显著提高。在20米分辨率下，仅使用强度数据的森林高度反演精度为3.1-3.8米，加入干涉相干性特征后提高到2.8米以下。在60米分辨率下，最佳RMSE达到1.95米。

Conclusion: 结合强度数据和干涉相干性特征的混合方法，配合注意力机制和嵌套连接融合，能够显著提高L波段SAR森林高度反演精度，适用于NISAR和未来ROSE-L任务。
...
TL;DR: 提出了一种轻量级的RIS相位设计框架，为6G网络中集成感知与通信系统提供闭式解，平衡通信与感知性能，并支持多目标感知的波束增益分布。


<details>
  <summary>Details</summary>
Motivation: 现有RIS优化方法主要基于半定松弛或迭代算法，前者计算复杂度高且可扩展性差，后者依赖初始值且只能获得次优解。需要一种更高效的方法来优化RIS配置。

Method: 将RIS配置分为两部分：第一部分最大化通信性能，第二部分引入小扰动为多目标感知生成多个波束，从而在通信和感知之间实现权衡。

Result: 仿真结果表明，所提方法在性能上与SDR相当，但计算复杂度显著降低。

Conclusion: 该轻量级框架为RIS相位设计提供了有效的闭式解，能够平衡通信与感知性能，同时支持多目标感知，计算效率高。
...
TL;DR: 本文提出了一种工作在20-100 GHz的开关型衰减器，采用电容补偿技术减小相位误差，使用金属线实现小电阻以减小寄生电容，并采用连续调谐衰减单元提高衰减精度。


<details>
  <summary>Details</summary>
Motivation: 设计一种在20-100 GHz宽频带范围内工作的衰减器，需要解决传统衰减器在高频段的相位误差大、寄生电容影响显著以及衰减精度不足的问题。

Method: 采用电容补偿技术减小相位误差；使用金属线实现小电阻以减小固有寄生电容；引入连续调谐衰减单元提高整体衰减精度；基于标准65nm CMOS工艺设计制造。

Result: 在20-100 GHz频段内实现7.5 dB的相对衰减范围，连续调谐步进；插入损耗为1.6-3.8 dB；所有状态的反射损耗优于11.5 dB；RMS幅度和相位误差分别低于0.15 dB和1.6°。

Conclusion: 该衰减器在20-100 GHz宽频带范围内实现了低插入损耗、良好的匹配性能以及高精度的衰减控制，验证了所提出的电容补偿和金属线电阻技术的有效性。
...
TL;DR: CardioPHON是一个集成的心脏声音质量评估和分类工具，用于从心音图记录中筛查异常心脏功能。该模型在六个心音数据集上以自监督方式预训练，能自动去除低质量录音，并在心音分类任务中达到最先进性能。
...
Method: 提出CardioPHON模型，在六个心音数据集上以自监督方式预训练，结合音频和社会人口学特征的多模态方法，以及仅基于心音图的单模态方法。

Result: 多模态模型在2022年George B. Moody PhysioNet心音挑战赛官方排行榜上获得最佳排名，单模态模型在单模态方法中排名第一（总排名第4），超越了使用多模态的模型。

Conclusion: CardioPHON是心音记录领域首个公开发布的预训练模型，有助于开发数据高效的人工智能模型，能够泛化到心血管诊断的各种下游任务。
...
TL;DR: MIDI-LLM是一个基于LLM的多轨MIDI音乐生成模型，能够从自由文本提示生成音乐，具有高质量、更好的文本控制和更快的推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有文本到MIDI音乐生成模型在质量和效率方面存在不足，需要开发一个能够从自由文本提示生成高质量多轨MIDI音乐的高效模型。

Method: 扩展文本LLM的词汇表以包含MIDI标记，采用两阶段训练方法赋予文本到MIDI的能力，同时保留原始LLM的参数结构以利用vLLM库进行加速推理。

Result: 实验表明，MIDI-LLM在质量、文本控制和推理速度方面均优于最近的Text2midi模型。

Conclusion: MIDI-LLM成功实现了从自由文本提示生成高质量多轨MIDI音乐的目标，在性能上超越了现有方法，并提供了实时演示。
...
TL;DR: MusRec是首个零样本文本到音乐编辑模型，能够在真实世界音乐上高效执行多样化编辑任务，解决了现有模型只能编辑自身生成音乐、需要精确提示或任务特定重训练的限制。


<details>
  <summary>Details</summary>
Motivation: 现有音乐编辑模型存在显著限制：只能编辑自身模型生成的合成音乐、需要高度精确的提示、或需要任务特定的重训练，缺乏真正的零样本能力。
...
TL;DR: PromptSep扩展了语言查询音频源分离(LASS)框架，通过条件扩散模型支持音频提取和声音移除，并引入人声模仿作为更直观的查询方式，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有语言查询音频源分离方法存在两个限制：(1)用户需要超越分离的操作，如声音移除；(2)仅依赖文本提示不够直观。

Method: 使用条件扩散模型结合精心设计的数据模拟，支持音频提取和声音移除，并引入人声模仿作为额外查询方式，采用Sketch2Sound作为数据增强策略。

Result: 在多个基准测试中，PromptSep在声音移除和人声模仿引导的源分离方面达到最先进性能，同时在语言查询源分离方面保持竞争力。

Conclusion: PromptSep成功扩展了LASS框架，提供更通用的声音分离能力，并通过多模态查询提高了系统的直观性和实用性。

...
TL;DR: 提出弹性网络拓扑(ENT)用于分布式ISAC服务，通过动态聚合CCN为CFN来平衡信令开销与资源利用，使用两阶段协议和MADRL框架优化USR
...
Method: 提出弹性网络拓扑(ENT)，允许CCN动态聚合为CFN；设计两阶段操作协议（服务分类与资源划分）；定义USR指标量化效用-信令权衡；采用MADRL框架进行集中训练分散执行

Result: 通过ENT架构和MADRL优化框架，能够有效平衡信令开销与分布式资源利用，实现高效的ISAC服务供给

Conclusion: 弹性网络拓扑为分布式ISAC服务提供了灵活高效的解决方案，通过动态聚合和资源协调机制解决了传统网络拓扑的刚性限制问题
...
TL;DR: 论文提出基于区块链和无人机验证的双层架构，用于灾害响应中的实时可信危机地图更新，通过数字投票和物理层验证结合解决数据可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 灾害响应中需要快速决策哪些区域需要紧急关注，但当前协调模型面临不可靠数据、故意错误信息和关键通信基础设施中断等问题，需要一种实时可信的协调机制。

Method: 采用区块链驱动的去中心化投票模型，结合无人机验证的双层架构：数字层通过区块链记录时间戳和地理标记的投票，物理层通过无人机进行物理层认证并发布独立位置标志到区块链，使用调制后向反射器传感器和3D感知的视距布置来最大化可验证性。

Result: 提出了一个融合不可变数字记录和感官基础信任的双签名机制，分析了从欺骗、投票模糊性到验证器妥协和连接丢失等核心技术和人本挑战，并概述了分层缓解策略。

Conclusion: 该架构为弹性、信任锚定的危机协调提供了一条路径，通过结合区块链的不可变性和无人机的物理验证，能够提高灾害响应中决策的可靠性和时效性。
...
Method: 采用集中式量子中心发射纠缠光子对，一个光子射向目标设备，另一个射向本地参考单元；目标设备配备平面CCR阵列被动反射光子，通过量子测量实现同步；建立了包含高斯光束扩展、空间失准、大气湍流和概率性光子生成等物理层现象的综合分析模型

Result: 推导了Gamma-Gamma衰落下的单光子检测概率闭式表达式，分析了背景光子、SPAD探测器抖动和量子生成随机性对同步精度和中断概率的影响；仿真验证了分析模型的准确性，揭示了光束腰、CCR阵列尺寸和背景光之间的关键权衡关系

Conclusion: 该架构为智慧城市安全定时提供了可行的低功耗、无基础设施解决方案，通过量子同步实现了亚纳秒级精度，具有良好的实际应用前景
...
TL;DR: 提出基于面积的信号噪声比(aSNR)作为传统基于峰值的pSNR的改进方案，通过积分信号区域来降低噪声方差，特别适用于宽峰检测


<details>
  <summary>Details</summary>
Motivation: 传统基于峰值的信号噪声比(pSNR)存在对噪声尖峰敏感、对宽峰检测效果差等局限性，需要更稳健的替代方案
...
Conclusion: aSNR是比pSNR更稳健的替代方案，可扩展到多维光谱的体积基SNR，为光谱分析提供了改进的检测性能
...
TL;DR: 提出一种用于光学LEO卫星网络的轻量级几何感知路由框架，通过DRL智能体结合抖动感知高斯光束模型，实现物理一致的路由选择


<details>
  <summary>Details</summary>
Motivation: 光学星间链路对卫星相对运动、指向抖动和快速变化的几何结构极其敏感，导致路由比RF系统更具挑战性，需要物理一致的路由策略

Method: 基于抖动感知高斯光束模型推导类相关可行性约束，将其嵌入时变ISL图中，通过深度强化学习智能体中的可行动作掩码强制执行，利用局部几何进展、可行邻居结构和拥塞指标选择下一跳中继

Result: 在类星链星座上的仿真结果表明，学习到的路径具有物理一致性，能够利用平面内稳定性，适应抖动限制的平面间连接性，并在动态拓扑演化下保持稳健的端到端延迟

Conclusion: 该几何感知路由框架能够有效处理光学LEO网络中的动态连接约束，实现物理一致且性能优越的路由决策
...
TL;DR: 这篇综述论文对自由空间光学、毫米波和太赫兹系统中的指向误差问题进行了统一处理，建立了跨技术的一致性术语和分类，并系统性地调查了缓解技术。


<details>
  <summary>Details</summary>
Motivation: 现有研究使用特定技术的定义、模型和缓解方法来处理指向误差问题，这阻碍了跨领域比较和可转移的设计见解。需要建立一个统一的框架来支持下一代定向通信系统的一致分析和稳健设计。

Method: 建立一致的术语和跨技术指向误差分类法，回顾角度失准和统计分布模型，分析其对系统性能的影响，并系统性地调查缓解技术，特别关注光学系统及其与底层指向误差模型的联系。

Result: 提供了指向误差在轨道角动量（OAM）链路和量子光通信中的详细检查，并调查了针对模式相关损伤和量子测量约束的相应缓解方法。将分散的文献整合到一个连贯的框架中。

Conclusion: 通过将分散的文献整合到一个连贯的框架中，这项工作支持下一代定向通信系统的一致分析和稳健设计，并概述了开放挑战和未来研究方向。
...
TL;DR: 本文系统综述了语义通信与无线接入网络的融合，提出了语义无线接入网络的概念，分析了其架构、各层关键技术、应用前景及研究挑战。


<details>
  <summary>Details</summary>
Motivation: 传统无线接入网络存在频谱效率低、复杂环境抗干扰能力弱、资源分配不灵活等问题，而语义通信通过传输意义而非比特，能显著提升通信性能。目前缺乏对语义通信与无线接入网络融合的全面综述，因此需要系统梳理这一新兴领域。

Method: 首先介绍RAN和语义通信的基础知识，分析传统RAN的局限性，提出语义RAN的整体架构。然后从物理层、数据链路层、网络层和安全层面综述代表性技术。最后展望未来应用和标准化进展。

Result: 提出了语义无线接入网络的完整架构，系统梳理了各层关键技术，包括物理层的语义编码、数据链路层的语义调度、网络层的语义路由等。展望了语义RAN在智能交通、工业物联网等领域的应用前景，并分析了当前的标准化进展。

Conclusion: 语义RAN代表了无线通信的重要范式转变，能显著提升频谱效率、抗干扰能力和用户体验。未来研究需解决语义建模、跨层优化、安全隐私等关键挑战，推动标准化进程，实现语义通信在无线接入网络中的广泛应用。
...
TL;DR: BenchLink是一个基于SoC的通信链路基准测试平台，能够在GPS拒止环境下实现精确的定时同步，支持自适应导频密度和调制，为无线通信研究提供硬件平台和数据集。


<details>
  <summary>Details</summary>
Motivation: 现代无线通信系统依赖GPS进行精确定时同步，但许多新兴应用需要在GPS信号不可靠或被干扰的环境中运行，导致振荡器漂移和载波频率损伤，需要解决GPS拒止环境下的通信同步问题。

Method: 采用基于SoC的设计（Zynq UltraScale+ MPSoCs），相比传统的GPP软件定义无线电（如USRP）能实现更精确的延迟控制，支持自适应导频密度和调制，在GPS拒止环境下实现稳健通信。

Result: 成功实现并评估了BenchLink系统，在地面和空中环境中均表现出有效性，收集了各种条件下的综合数据集，并将SoC链路设计和数据集开源给无线社区。

Conclusion: BenchLink将为GPS拒止场景下的数据驱动链路自适应、稳健同步研究提供支持，并促进需要精确延迟控制的新兴应用发展，如集成雷达感知与通信。
...
TL;DR: RIS技术通过虚拟视距连接和角度选择性干扰重塑，解决卫星网络中的遮挡中断和频谱共存问题，提升系统性能。


<details>
  <summary>Details</summary>
Motivation: 卫星网络面临两大挑战：遮挡导致的服务中断，以及卫星层间和星地系统间日益严格的频谱共存需求。需要低功耗、可编程的解决方案来提升网络可靠性和频谱效率。

Method: 采用可重构智能表面(RIS)作为低功耗可编程孔径，将其视为卫星/终端天线以及星间或星地中继。通过虚拟视距连接恢复遮挡下的连通性，利用角度选择性重塑干扰以扩大频谱复用。

Result: 系统级增益由两个统一机制驱动：通过虚拟视距链路在遮挡和移动下保持连通性，以及通过角度选择性重塑干扰以扩大频谱复用。讨论了高移动性下的实际操作挑战和解决方案。
...
TL;DR: 本文研究了一种低空集成感知与通信系统，利用可旋转的有源和无源阵列协同工作，通过联合优化基站预编码器、RIS相移和阵列旋转角度来最大化下行链路和速率，同时满足感知需求。


<details>
  <summary>Details</summary>
Motivation: 在低空ISAC系统中，传统固定阵列的几何限制会影响系统性能。本文旨在通过引入可旋转的基站和RIS阵列，利用三维方向性来改善通信和感知性能，特别是在低空几何环境中。

Method: 提出了一种基于惩罚的交替优化框架：1) 使用二次变换和MM算法优化基站预编码器；2) 在单位模流形上使用黎曼共轭梯度优化RIS相移；3) 使用带Barzilai-Borwein步长的投影梯度下降优化BS/RIS阵列旋转角度。

Result: 在低空几何环境中，所提出的联合可旋转BS-RIS架构相比固定或部分可旋转基线实现了显著的和速率增益，特别是在使用定向天线和干扰受限场景下，同时保证了感知要求。

Conclusion: 通过联合优化BS和RIS的机械旋转，可以显著提升低空ISAC系统的性能，证明了三维阵列方向性在改善通信和感知性能方面的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 随着LEO星座的快速扩张，数千颗卫星搭载GNSS接收机形成了前所未有的星载GNSS网络。然而当前星载GNSS处理多为独立运行，精度不足，而集中式融合面临计算瓶颈和星上基础设施缺乏的挑战。
...
TL;DR: 研究评估了芬兰奥卢大学医院手术室部署私有5G网络的可行性，结果显示与现有LTE/Wi-Fi无干扰，且射频暴露远低于安全标准，为医院5G部署提供了安全基础。


<details>
  <summary>Details</summary>
Motivation: 医院环境对无线通信有特殊要求，需要评估私有5G网络在3.9-4.1 GHz频段部署时是否会对现有LTE和Wi-Fi系统造成干扰，以及电磁辐射是否符合患者和医护人员的安全标准。

Method: 在奥卢大学医院手术室进行测量活动，采用先进仪器和高分辨率、长时间频谱扫描方法，评估5G网络与相邻LTE/Wi-Fi频段的干扰情况，并测量射频暴露水平。

Result: 测量显示医院私有5G网络与相邻LTE（4G）和Wi-Fi频段之间无可测量干扰，证实了频谱隔离性；手术室射频暴露水平远低于ICNIRP、WHO和IEEE的安全阈值。

Conclusion: 研究为医院环境集成私有5G基础设施提供了坚实基础，支持医疗数字化转型而不影响电磁兼容性或患者安全，并提出了频谱管理策略和可操作指南。
...
TL;DR: 研究毫米波频段中的频谱移动性和天线阵列管理，通过POMDP和PBVI算法优化波束成形，提高移动场景下的数据速率。


<details>
  <summary>Details</summary>
Motivation: 毫米波FR2（24-71 GHz）和新兴FR3（7-24 GHz）频段虽然带宽大，但高衰减需要定向波束。在车辆连接等高移动性场景中，多频段下的天线和波束管理面临挑战，需要更鲁棒高效的波束成形方案。

Method: 将频谱移动性问题建模为部分可观测马尔可夫决策过程（POMDP），使用基于点的值迭代（PBVI）算法寻找具有性能保证的策略，以在每个时隙选择最优信道频率和波束方向。

Result: 数值实验证实了所提策略在多个可用频段下的有效性，即使在用户移动性显著偏离策略生成时假设的模型情况下，仍能保持良好性能。

Conclusion: 通过频谱移动性和天线阵列管理的结合，能够显著改善移动环境下的波束成形性能，为高移动性毫米波通信提供有效的解决方案。
...
TL;DR: 提出ResNet-SA框架，通过残差学习网络结合模拟退火算法，解决可重构智能表面中理论天线模型与实际天线模式不匹配问题，提升干扰抑制性能。


<details>
  <summary>Details</summary>
Motivation: 可重构智能表面在大型反射面天线系统中用于空间干扰抑制，但现有算法依赖精确的理论辐射模型。实际中反射面天线畸变会导致理论模型与实际天线模式不匹配，直接应用优化权重会导致干扰消除性能下降。

Method: 提出残差学习网络辅助的模拟退火框架（ResNet-SA），通过神经网络学习理论天线增益与实际天线增益之间的残差差异，将神经网络嵌入启发式优化算法中寻找最优权重向量，无需显式了解畸变电场。

Result: 仿真结果表明，与传统仅基于理论模型优化权重的方法相比，所提方法在真实辐射模式中实现了更深的零陷深度，验证了ResNet-SA算法在仅近似了解天线模式情况下的有效性。

Conclusion: ResNet-SA框架能有效解决可重构智能表面中理论模型与实际天线模式不匹配问题，通过残差学习和启发式优化相结合，在不需精确畸变电场知识的情况下提升干扰抑制性能。
...
TL;DR: GenTSE是一个两阶段解码器生成语言模型，用于目标说话人提取，通过分离语义和声学表示实现更稳定解码和更忠实的目标语音


<details>
  <summary>Details</summary>
Motivation: 基于语言模型的生成建模为目标说话人提取提供了改进泛化能力和高保真语音的潜力，但需要解决解码稳定性和输出质量的问题

Method: 两阶段解码器生成语言模型：第一阶段预测粗粒度语义token，第二阶段生成细粒度声学token；使用连续SSL或codec嵌入；采用Frozen-LM Conditioning训练策略减少曝光偏差；使用DPO对齐人类感知偏好

Result: 在Libri2Mix数据集上，GenTSE在语音质量、可懂度和说话人一致性方面超越了之前的基于语言模型的系统

Conclusion: 分离语义和声学表示的两阶段生成方法能够稳定解码并产生更忠实的内容对齐目标语音，结合连续嵌入和减少曝光偏差的训练策略显著提升了目标说话人提取的性能
...
TL;DR: 提出一个统一框架，结合声音分离和目标声音提取，通过编码器-解码器吸引子网络自动推断声源数量和声学线索，多模态融合网络精确解释用户提供的线索，实现自适应工作模式。


<details>
  <summary>Details</summary>
Motivation: 现有声音分离方法难以确定未知声源数量，目标声音提取方法需要精确指定的线索才能达到最佳性能。需要克服这两种方法的各自局限性。

Method: 1) 编码器-解码器吸引子网络自动推断声源数量和相应声学线索；2) 多模态融合网络精确解释用户提供的声学、语义或视觉线索；通过跨任务一致性约束的联合训练，建立统一潜在空间。

Result: 实验显示在两个任务中都取得了显著性能：声音分离相比基线提升了1.4 dB SDR，目标声音提取准确率达到86%。

Conclusion: 提出的统一框架成功结合了声音分离和目标声音提取的优势，能够自适应地在完全自主的声音分离模式或线索驱动的目标声音提取模式下工作，克服了各自方法的局限性。
...
TL;DR: SACodec：基于语义锚定机制的新型神经语音编解码器，通过非对称双量化器在1.5kbps下同时保持声学保真度和语义丰富性


<details>
  <summary>Details</summary>
Motivation: 传统神经语音编解码器在低比特率下面临基本权衡：保持声学保真度往往会损害语义丰富性。需要一种方法能够同时保留语音的声学细节和语义信息。

Method: 提出SACodec，基于非对称双量化器架构，采用语义锚定机制。使用轻量级投影器将声学特征与冻结的大规模mHuBERT码本对齐，注入语言先验并确保码本完全利用。然后通过带有SimVQ的残差激活模块，让单层量化器（声学路径）恢复细粒度信息。

Result: 在1.5kbps比特率下，SACodec在保真度和语义丰富性方面都达到了新的最先进水平。主观听力测试显示其重建质量在感知上与原始音频高度可比，同时其token在下游任务中表现出显著改善的语义丰富性。
...
TL;DR: 提出基于半监督机器学习的自动钢琴缩编方法，通过音乐简化与和声化两步流程，利用MidiBERT框架实现，仅需少量标注数据即可生成实用的钢琴缩编结果。


<details>
  <summary>Details</summary>
Motivation: 钢琴缩编是重要的音乐转换过程，但手动制作耗时耗力。传统监督学习需要大量标注数据，而音乐领域标注数据稀缺。本研究旨在利用半监督学习，充分利用古典音乐中的丰富数据，减少标注需求。

Method: 采用两步法：音乐简化后和声化。基于现有机器学习框架MidiBERT，提出并实现了两种解决方案，利用半监督学习处理钢琴缩编任务。

Result: 方法能够输出实用且真实的钢琴缩编样本，准确度高，仅需少量后处理调整。为半监督学习在自动钢琴缩编领域的应用奠定了基础。

Conclusion: 本研究为自动钢琴缩编的半监督学习方法提供了基础框架，未来研究者可在此基础上开发更先进的技术。证明了利用丰富音乐数据减少标注需求的可行性。

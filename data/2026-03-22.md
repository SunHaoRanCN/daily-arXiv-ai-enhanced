<div id=toc></div>

# Table of Contents

- [eess.SP](#eess.SP) [Total: 13]
- [eess.AS](#eess.AS) [Total: 4]
- [cs.SD](#cs.SD) [Total: 5]


<div id='eess.SP'></div>

# eess.SP [[Back]](#toc)

### [1] [Physically Accurate Differentiable Inverse Rendering for Radio Frequency Digital Twin](https://arxiv.org/abs/2603.18026)
*Xingyu Chen,Xinyu Zhang,Kai Zheng,Xinmin Fang,Tzu-Mao Li,Chris Xiaoxuan Lu,Zhengxiong Li*

Main category: eess.SP

TL;DR: RFDT是一个基于物理的可微分射频仿真框架，解决了传统RF模拟器不可微分的问题，实现了虚拟与物理世界之间的梯度交互。


<details>
  <summary>Details</summary>
Motivation: 数字孪生在RF系统中的潜力受到传统RF模拟器不可微分性质的限制。传播路径的可见性导致严重的不连续性，计算机图形学中的可微分渲染技术难以直接迁移到RF领域。

Method: RFDT采用物理基础的边缘衍射过渡函数解决不连续性问题，并通过信号域变换代理缓解傅里叶域处理带来的非凸性。

Result: RFDT能够从真实RF测量中准确重建数字孪生，并能增强下游应用，如基于机器学习的RF传感的测试时适应和通信系统的物理约束优化。

Conclusion: RFDT提供了一个可微分的RF仿真框架，克服了传统RF模拟器的局限性，实现了虚拟与物理世界之间的梯度交互，为RF系统的数字孪生应用开辟了新途径。

Abstract: Digital twins, virtual simulated replicas of physical scenes, are transforming system design across industries. However, their potential in radio frequency (RF) systems has been limited by the non-differentiable nature of conventional RF simulators. The visibility of propagation paths causes severe discontinuities, and differentiable rendering techniques from computer graphics cannot easily transfer due to point-source antennas and dominant specular reflections. In this paper, we present RFDT, a physically based differentiable RF simulation framework that enables gradient-based interaction between virtual and physical worlds. RFDT resolves discontinuities with a physically grounded edge-diffraction transition function, and mitigates non-convexity from Fourier-domain processing through a signal domain transform surrogate. Our implementation demonstrates RFDT's ability to accurately reconstruct digital twins from real RF measurements. Moreover, RFDT can augment diverse downstream applications, such as test-time adaptation of machine learning-based RF sensing and physically constrained optimization of communication systems.

</details>


### [2] [KD-EKF: Knowledge-Distilled Adaptive Covariance EKF for Robust UWB/PDR Indoor Localization](https://arxiv.org/abs/2603.18027)
*Kyeonghyun Yoo,Wooyong Jung,Namkyung Yoon,Sangmin Lee,Sanghong Kim,Hwangnam Kim*

Main category: eess.SP

TL;DR: 提出KD-EKF框架，通过知识蒸馏学习历史轨迹的可靠性线索，自适应调节EKF测量协方差，提升UWB/PDR融合定位在异构环境下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: UWB在NLOS条件下测量可靠性严重下降，IMU-PDR存在误差累积问题。传统EKF/PF融合方法依赖手动调参的固定测量协方差，难以适应不同室内布局、NLOS比例和运动模式，导致测量不确定性建模在异构环境中泛化能力差。

Method: 提出自适应测量协方差缩放框架：1) 离线使用大型教师模型从结构化UWB/PDR序列生成时间一致的下一个位置预测；2) 将教师模型行为蒸馏到适合实时部署的轻量级学生模型；3) 学生模型基于预测残差连续调节EKF测量协方差，实现环境感知融合而无需手动重新调参。

Result: KD-EKF框架显著降低定位误差，抑制LOS/NLOS转换期间的误差尖峰，减轻长期漂移，相比固定参数EKF在不同室内环境中提高了测量鲁棒性。

Conclusion: 通过知识蒸馏学习历史轨迹可靠性线索的自适应协方差调节方法，能够有效解决传统UWB/PDR融合定位中固定参数难以适应异构环境的问题，提升定位系统的鲁棒性和泛化能力。

Abstract: Ultra-wideband (UWB) indoor localization provides centimeter-level accuracy and low latency, but its measurement reliability degrades severely under Non-Line-of-Sight (NLOS) conditions, leading to meter-scale ranging errors and inconsistent uncertainty characteristics. Inertial Measurement Unit (IMU)-based Pedestrian Dead Reckoning (PDR) complements UWB by providing infrastructure-free motion estimation; however, its error accumulates nonlinearly over time due to bias and noise propagation. Fusion methods based on Extended Kalman Filters (EKF) and Particle Filters (PF) can improve average localization accuracy through probabilistic state estimation. However, these approaches typically rely on manually tuned measurement covariances. Such fixed or heuristically tuned parameters are hard to sustain across varying indoor layouts, NLOS ratios, and motion patterns, leading to limited robustness and poor generalization of measurement uncertainty modeling in heterogeneous environments. To address this limitation, this work proposes an adaptive measurement covariance scaling framework in which reliability cues are learned from historical UWB/PDR trajectories. A large teacher model is employed offline to generate temporally consistent next-position predictions from structured UWB/PDR sequences, and this behavior is distilled into a lightweight student model suitable for real-time deployment. The student model continuously regulates EKF measurement covariances based on prediction residuals, enabling environment-aware fusion without manual re-tuning. Experimental results demonstrate that the proposed KD-EKF framework significantly reduces localization error, suppresses error spikes during Line-of-Sight (LOS)/NLOS transitions, and mitigates long-term drift compared to fixed-parameter EKF, thereby improving measurement robustness across diverse indoor environments.

</details>


### [3] [Enabling 6G Wireless Communications: UWB Characterization of Corridors within the H-Band](https://arxiv.org/abs/2603.18629)
*Juan E. Galeote-Cazorla,Alejandro Ramírez-Arroyo,Mauricio Rodríguez,Reinaldo Valenzuela,Juan F. Valenzuela-Valdés*

Main category: eess.SP

TL;DR: 该论文研究了250-330 GHz H频段太赫兹波在走廊环境中的传播特性，首次同时分析了80 GHz超宽带信道，发现路径增益指数在-2.1到-1.6之间，表明存在波导效应，适合部署点对点链路。


<details>
  <summary>Details</summary>
Motivation: 第六代无线系统需要达到1 Tbps的数据速率和低于1 ms的延迟，太赫兹频段是实现这一目标的有前景技术。但由于高频段的高损耗和传播机制受限，部署面临挑战，因此需要研究短距离场景下的太赫兹传播特性。

Method: 在走廊环境中进行广泛的测量活动，频率范围为250-330 GHz的H频段，首次同时研究80 GHz超宽带。评估大尺度效应（路径增益建模）和小尺度效应（莱斯K因子、均方根时延扩展、相干带宽）。提出基于经典N射线模型的解析近似方法。

Result: 路径增益指数在-2.1到-1.6之间，表明存在波导效应（类似毫米波频段观察到的现象）。小尺度参数分析显示视距分量占主导地位。提出的解析模型与经验分析一致，能准确表示无线信道。

Conclusion: 太赫兹频段在走廊环境中适合部署点对点链路，因为视距分量相对于反射分量占主导地位。研究结果为太赫兹频段在第六代无线系统中的应用提供了重要参考。

Abstract: Future sixth-generation of wireless system is expected to provide data-rates in the order of 1 Tbps and latencies below 1 ms. Among others, one of the most promising strategies to meet these requirements is to operate at higher frequencies than millimeter wave bands: the THz bands. Nevertheless, because of the higher losses and the detriment of classical propagation mechanisms, deploying systems operating at these frequencies becomes a real challenge. Consequently, short-range scenarios are of special interest since these effects of THz bands can be managed. This work conducts an extensive campaign within corridors at frequencies within the H-band in the range from 250 GHz to 330 GHz. For the first time in literature, an ultra wideband of 80 GHz is studied simultaneously. Large scale effects are assessed by estimating and modeling path gain. The path gain exponent varies between -2.1 and -1.6, which is explained by a guiding effect also observed at millimeter wave bands. Small scale effects are also evaluated in terms of parameters such as rice $K$-factor, root mean squared delay spread and coherence bandwidth. Additionally, an analytical approximation based on the classical N-rays model is proposed obtaining an accurate representation of the wireless channel which is coherent with the empirical analysis. The full analysis reveals the suitability of these THz bands for deploying point-to-point links due to the predominance of the line-of-sight contribution respect to the reflected components.

</details>


### [4] [Secure Cell-Free Massive MIMO ISAC Systems: Joint AP Selection and Power Allocation Against Eavesdropping](https://arxiv.org/abs/2603.18635)
*Ruiguang Wang,Takumi Takahashi,Hideki Ochiai*

Main category: eess.SP

TL;DR: 提出一种面向CF-mMIMO ISAC系统的安全增强方案，通过AP分组策略和联合优化框架，在感知区域内抑制窃听者，平衡通信效率、感知精度与安全性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 针对CF-mMIMO ISAC系统中，位于感知区域内的潜在窃听者可能导致信息泄露的关键安全问题，需要设计有效的安全防护机制。

Method: 提出AP分组策略：将分布式AP分为通信AP（C-AP）和感知AP（S-AP），分别负责数据传输与目标检测/窃听抑制。采用连续凸近似（SCA）方法，构建两个互补的优化问题：1）安全约束下的用户速率最大化；2）QoS要求下的窃听速率最小化。

Result: 推导了可达通信速率、窃听速率和主瓣平均旁瓣比（MASR）的闭式表达式。数值结果表明，所提方案在通信效率、感知精度和安全性方面取得显著性能增益，揭示了这三者之间的内在权衡关系。

Conclusion: 提出的联合优化框架通过AP操作模式选择和功率分配，为设计安全的CF-mMIMO ISAC系统提供了实用指导，有效解决了感知区域内的窃听安全问题。

Abstract: This paper investigates a cell-free massive multiple-input-multiple-output (CF-mMIMO) integrated sensing and communication (ISAC) system that addresses the critical challenge of information leakage to potential eavesdroppers located within sensing zones. A novel access point (AP) selection strategy is proposed, which partitions the distributed APs into two functional groups: communication APs (C-APs), dedicated exclusively to data transmission, and sensing APs (S-APs), responsible for target detection and eavesdropper suppression. Closed-form expressions for the achievable communication rate, eavesdropping rate, and mainlobe-to-average-sidelobe ratio (MASR) are derived to evaluate system performance. Two complementary optimization problems are formulated using the successive convex approximation (SCA): (i) maximizing user rates under security constraints and (ii) minimizing eavesdropping rates while satisfying quality of service (QoS) requirements. The proposed joint optimization framework determines the optimal AP operational modes and power allocation across communication and sensing links. Extensive numerical results validate the theoretical analysis and demonstrate significant performance gains, revealing inherent trade-offs among communication efficiency, sensing accuracy, and security. These insights offer practical guidelines for designing secure CF-mMIMO ISAC systems.

</details>


### [5] [Holter-to-Sleep: AI-Enabled Repurposing of Single-Lead ECG for Sleep Phenotyping](https://arxiv.org/abs/2603.18714)
*Donglin Xie,Qingshuo Zhao,Jingyu Wang,Shijia Geng,Jiarui Jin,Jun Li,Rongrong Guo,Guangkun Nie,Gongzheng Tang,Yuxi Zhou,Thomas Penzel,Shenda Hong*

Main category: eess.SP

TL;DR: 提出一个基于单导联心电图的Holter-to-Sleep框架，能同时进行睡眠表型分析和Holter级心脏表型分析，为大规模心脏-睡眠关联研究提供可扩展方案。


<details>
  <summary>Details</summary>
Motivation: 传统多导睡眠图(PSG)资源密集，不适合多夜、家庭和大规模筛查。单导联心电图在Holter和贴片设备中已普及，能舒适地进行长期采集，并通过自主神经调节和心肺耦合编码睡眠相关生理信息。

Method: 开发并验证Holter-to-Sleep框架，仅使用单导联心电图作为输入，联合支持过夜睡眠表型分析和Holter级心脏表型分析。在包含10,439项研究的四个公共队列的多中心PSG样本上进行开发和验证，并进行独立外部评估以评估跨队列泛化能力，同时使用过夜贴片心电图记录进行客观-主观一致性分析的现实可行性评估。

Result: 该集成设计能够在异质人群和采集条件下稳健提取具有临床意义的过夜睡眠表型，并促进心电图衍生的睡眠指标与心律失常相关的Holter表型之间的系统关联。

Conclusion: Holter-to-Sleep范式为低负担、可家庭部署、可扩展的心脏-睡眠监测和研究提供了实用基础，超越了传统的以PSG为中心的工作流程。

Abstract: Sleep disturbances are tightly linked to cardiovascular risk, yet polysomnography (PSG)-the clinical reference standard-remains resource-intensive and poorly suited for multi-night, home-based, and large-scale screening. Single-lead electrocardiography (ECG), already ubiquitous in Holter and patch-based devices, enables comfortable long-term acquisition and encodes sleep-relevant physiology through autonomic modulation and cardiorespiratory coupling. Here, we present a proof-of-concept Holter-to-Sleep framework that, using single-lead ECG as the sole input, jointly supports overnight sleep phenotyping and Holter-grade cardiac phenotyping within the same recording, and further provides an explicit analytic pathway for scalable cardio-sleep association studies. The framework is developed and validated on a pooled multi-center PSG sample of 10,439 studies spanning four public cohorts, with independent external evaluation to assess cross-cohort generalizability, and additional real-world feasibility assessment using overnight patch-ECG recordings via objective-subjective consistency analysis. This integrated design enables robust extraction of clinically meaningful overnight sleep phenotypes under heterogeneous populations and acquisition conditions, and facilitates systematic linkage between ECG-derived sleep metrics and arrhythmia-related Holter phenotypes. Collectively, the Holter-to-Sleep paradigm offers a practical foundation for low-burden, home-deployable, and scalable cardio-sleep monitoring and research beyond traditional PSG-centric workflows.

</details>


### [6] [Angularly-Resolved 3D Foliage Modeling and Measurements at 60 and 80 GHz: From Stochastic Geometry to Deterministic Channel Characterization](https://arxiv.org/abs/2603.18785)
*Jiri Blumenstein,Radek Zavorka,Josef Vychodil,Tomas Mikulasek,Jaroslaw Wojtun,Jan M. Kelner,Cezary Ziolkowski,Rajeev Shukla,Markus Hofer,Thomas Zemen,Christoph F Mecklenbrauker,Aniruddha Chandra,Ales Prokes*

Main category: eess.SP

TL;DR: 提出了一种生成树叶3D模型的随机方法，用于确定性射线追踪信道建模，并在60和80 GHz频段通过测量验证


<details>
  <summary>Details</summary>
Motivation: 需要一种能够准确模拟树叶对毫米波信道影响的3D建模方法，以支持确定性射线追踪信道建模

Method: 采用随机方法生成树叶3D模型，模型包含树体积、叶片大小、叶片密度和树冠形状等可解释参数，结合电磁材料特性，用于确定性射线追踪信道建模

Result: 通过60和80 GHz频段（2 GHz带宽）的测量验证了该方法，分析了路径损耗和RMS时延扩展，并展示了接收器围绕树木半圆形移动时这些参数的角相关性

Conclusion: 提出的随机3D树叶建模方法能够有效支持毫米波频段的确定性射线追踪信道建模，模型参数具有物理可解释性

Abstract: In this paper, we show a stochastic approach to generate a 3D model of a foliage, which is then used for deterministic ray-tracing channel modeling. This approach is verified by a measurement campaign at 60 and 80 GHz with 2 GHz bandwidth. The wireless channel is characterized by path-loss and RMS delay spread and we show the angular dependency of those parameters when the receiver is placed on a half-circle around the tree. Besides electromagnetic material properties, the 3D model is characterized by several interpretable parameters, including tree volume, leaf size, leaf density, and the tree crown shape parameter.

</details>


### [7] [Stochastic 3-D Foliage Modeling at 80 GHz: Experimental Validation and Ray-Tracing Simulations](https://arxiv.org/abs/2603.18810)
*Jiri Blumenstein,Radek Zavorka,Josef Vychodil,Tomas Mikulasek,Jaroslaw Wojtun,Jan M. Kelner,Cezary Ziolkowski,Rajeev Shukla,Markus Hofer,Thomas Zemen,Christoph Mecklenbrauker,Aniruddha Chandra,Ales Prokes*

Main category: eess.SP

TL;DR: 提出一种用于3D树叶的随机建模方法，旨在增强光线追踪模拟，支持可调几何、密度和形状参数，在80GHz频段验证了模型对路径损耗和延迟扩展的影响


<details>
  <summary>Details</summary>
Motivation: 为了改进光线追踪模拟中树叶结构的建模精度，需要能够捕捉树叶结构变异性的随机模型，特别是在毫米波频段（80GHz）对无线传播特性的影响分析

Method: 开发了一种支持可调随机几何、密度和形状的3D树叶随机建模方法，通过代表性植被的实验测量进行验证，并分析树叶密度和大小对传播参数的影响

Result: 模型成功验证了树叶密度和大小对80GHz频段路径损耗和均方根延迟扩展的影响，展示了模型在毫米波无线传播分析中的适用性

Conclusion: 提出的3D树叶随机建模方法能有效增强光线追踪模拟，为毫米波频段的植被传播特性分析提供了实用工具

Abstract: A stochastic modeling methodology for 3-D foliage is presented, aimed at enhancing ray-tracing simulations. The model supports adjustable stochastic geometry, density, and shape to capture variability in foliage structures. The model is validated through experimental measurements of representative vegetation. The influence of foliage density and size on path loss and root mean square delay spread is analyzed to demonstrate the applicability of the model in the 80 GHz frequency band.

</details>


### [8] [Microdiversity and Vegetation Influence on Forward Scattering at 60 GHz and 80 GHz](https://arxiv.org/abs/2603.18831)
*Radek Zavorka,Ondrej Zeleny,Jiri Blumenstein,Tomas Mikulasek,Rajeev Shukla,Josef Vychodil,Jaroslaw Wojtun,Niraj Narayan,Aniruddha Chandra,Jan M. Kelner,Cezary Ziolkowski,Ales Prokes*

Main category: eess.SP

TL;DR: 实验研究60GHz和80GHz频段在植被存在下的信号传播特性，重点关注前向散射和微多样性效应，发现80GHz对天线微位移更敏感


<details>
  <summary>Details</summary>
Motivation: 理解植被和小尺度天线移动对信号传播的影响对于高频无线通信系统的设计和优化至关重要

Method: 在室内环境中进行受控测量，将盆栽植物置于发射器和接收器之间的视距路径中，研究天线微位移对信道脉冲响应的影响

Result: 80GHz频段比60GHz频段对微位移更敏感，导致接收功率波动更大

Conclusion: 高频无线通信系统设计需要考虑植被影响和天线微位移带来的信号波动，特别是80GHz频段对微小位置变化更为敏感

Abstract: Understanding the impact of vegetation and small-scale antenna movements on signal propagation is important for the design and optimization of high-frequency wireless communication systems. This paper presents an experimental study analyzing signal propagation at 60 GHz and 80 GHz in the presence of vegetation, with a focus on forward scattering and microdiversity effects. A controlled measurement campaign was conducted in an indoor environment, where the influence of a potted plant placed in the line-of-sight (LOS) path between the transmitter and receiver was investigated. The study examines the effects of antenna micro-shifts on the channel impulse response (CIR), highlighting variations in received power due to small positional changes of the antennas. The results indicate that the 80 GHz band exhibits higher sensitivity to micro-movements compared to the 60 GHz band, leading to greater fluctuations in received power.

</details>


### [9] [PAPR-Aware Waveform Design for Energy-Efficient MIMO-OFDM SWIPT](https://arxiv.org/abs/2603.18851)
*Chongda Huang,Yue Xiao,Qianzhen Zhang,Lilin Dan,Xianfu Lei,Kai-Kit Wong*

Main category: eess.SP

TL;DR: 提出了一种用于MIMO-OFDM SWIPT系统的统一分析框架，通过波形统计特性优化PA效率和整流器输出，采用频域资源分配策略平衡通信吞吐量和能量收集性能。


<details>
  <summary>Details</summary>
Motivation: SWIPT系统中波形设计对可靠数据传输和高效能量收集至关重要。PAPR存在双重作用：低PAPR提高功率放大器效率，高PAPR利用整流器非线性提升能量收集效率，这种矛盾使得PAPR成为SWIPT系统设计的关键挑战。

Method: 建立统一分析框架，表征PAPR对PA和整流器行为的影响；提出频域资源分配策略，自适应分配频谱段以平衡通信吞吐量和能量收集性能；将SWIPT扩展到MIMO-OFDM架构，通过波形适配和资源优化解决大规模天线-子载波配置中的高PAPR问题。

Result: 仿真结果表明，所提方法显著提高了PA效率、整流器输出和整体能量传输效率，验证了MIMO-OFDM作为联合数据传输和能量传输的高效能平台的可行性。

Conclusion: 通过适当的波形适配和资源优化，MIMO-OFDM可以成为高效的数据和能量联合传输平台，解决了SWIPT系统中PAPR设计的根本挑战。

Abstract: Simultaneous wireless information and power transfer (SWIPT) critically depends on waveform design, which governs both reliable data delivery and efficient energy harvesting. Among waveform characteristics, the peak-to-average power ratio (PAPR) plays a pivotal role: low-PAPR signals improve power amplifier (PA) efficiency, while high-PAPR signals exploit rectifier nonlinearities to boost harvested energy. This duality makes PAPR a fundamental design challenge in SWIPT systems. To tackle this issue, we establish a unified analytical framework that characterizes the PAPR-dependent behaviors of both the PA and the rectifier, thereby revealing how waveform statistics determine end-to-end energy transfer efficiency. Building on this insight, we propose a frequency-domain resource allocation strategy for power-splitting SWIPT, where spectral segments are adaptively assigned to balance communication throughput with energy harvesting performance. Here, a key contribution is to extend SWIPT to MIMO-OFDM architectures. Despite concerns over excessive PAPR in large-scale antenna-subcarrier configurations, we demonstrate that appropriate waveform adaptation and resource optimization can transform MIMO-OFDM into an energy-efficient platform for joint data and power transfer. Finally, simulation results confirm significant improvements in PA efficiency, rectifier output, and overall energy transfer, thereby validating the practical benefits of the proposed approach.

</details>


### [10] [Radar Detection through Rectified Flow Matching](https://arxiv.org/abs/2603.18995)
*P. Meena,Y. A. Rouzoumka,J. Pinsolle,C. Ren,M. N. El Korso,J. -P. Ovarlez*

Main category: eess.SP

TL;DR: 提出基于整流流匹配的雷达检测方法D-RFM，通过学习从高斯分布到雷达观测的映射，在非高斯杂波和热噪声混合环境下实现目标检测。


<details>
  <summary>Details</summary>
Motivation: 在非高斯杂波和白热噪声混合环境下进行雷达目标检测是一个具有挑战性的问题，现有检测器难以有效处理这种复杂场景。

Method: 提出D-RFM方法，通过学习从标准高斯分布到雷达观测的映射，捕获底层速度场。检测时通过学习的速度场将测试样本反向映射到潜在高斯空间，将目标识别为与学习分布的偏差。

Result: 实验结果表明，该方法在高斯和非高斯杂波加加性白高斯噪声环境下均表现出色，具有高准确性、鲁棒性和计算效率。

Conclusion: D-RFM方法为复杂杂波环境下的雷达目标检测提供了一种有效解决方案，通过学习数据分布实现准确的目标识别。

Abstract: Radar target detection in the presence of a mixture of non-Gaussian clutter and white thermal noise is a challenging problem. This paper proposes a Rectified Flow Matching-based method for radar detection, termed D-RFM. Unlike existing detectors, D-RFM learns a mapping from a standard Gaussian distribution to radar observations by capturing the underlying velocity field. Detection is then performed by inverse mapping test samples into the latent Gaussian space using the learned velocity field, with targets identified as deviations from the learned distribution. Experimental results demonstrate the efficacy of the proposed method under both Gaussian and non-Gaussian clutter plus additive white Gaussian noise, highlighting its accuracy, robustness, and computational efficiency.

</details>


### [11] [Mobile Radio Networks and Weather Radars Dualism: Rainfall Measurement Revolution in Densely Populated Areas](https://arxiv.org/abs/2603.19153)
*Davide Tornielli Bellini,Mario Montopoli,Dario Tagliaferri,Luca Baldini,Elisa Adirosi,Sergi Duque,Laura Resteghini,Umberto Spagnolini*

Main category: eess.SP

TL;DR: 利用蜂窝基站网络作为分布式机会雷达进行降雨遥感，首次实现城市环境下的高时空分辨率降雨监测


<details>
  <summary>Details</summary>
Motivation: 传统天气雷达在城区空间分辨率有限，而蜂窝基站网络密度高且具备电子可控天线阵列等技术特性，为城市水文气象监测提供了新的机会

Method: 将多普勒天气雷达信号处理技术应用于基站信号，通过适当的杂波滤波处理，从基站信号中提取反射率因子、平均多普勒速度和谱宽等典型天气雷达产品

Result: 该方法实现了前所未有的空间分辨率（几米级）和时间分辨率（数十秒级），杂波滤波后的雷达参数质量达到满意水平，与原始观测和重叠视场独立基站测量结果一致

Conclusion: 利用现有电信基础设施进行降雨遥感具有变革性意义，为城市水文气象提供了前所未有的空间粒度和时间即时性的降雨信息

Abstract: This study demonstrates, for the first time, how a network of cellular base stations (BSs) - the infrastructure of mobile radio networks - can be used as a distributed opportunistic radar for rainfall remote sensing. By adapting signal-processing techniques traditionally employed in Doppler weather radar systems, we demonstrate that BS signals can be used to retrieve typical weather radar products, including reflectivity factor, mean Doppler velocity, and spectral width. Due to the high spatial density of BS infrastructure in urban environments, combined with intrinsic technical features such as electronically steerable antenna arrays and wide receiver bandwidths, the proposed approach achieves unprecedented spatial and temporal resolutions, on the order of a few meters and several tens of seconds, respectively. Despite limitations related to low transmitted power, limited antenna gain, and other system constraints, a major challenge arises from ground clutter contamination, which is exacerbated by the nearly horizontal orientation of BS antenna beams. This work provides a thorough assessment of clutter impact and demonstrates that, through appropriate processing, the resulting clutter-filtered radar moments reach a satisfactory level of quality when compared with raw observations and with measurements from independent BSs with overlapped field-of-views. The findings highlight a transformative opportunity for urban hydrometeorology: leveraging existing telecommunications infrastructure to obtain rainfall information with a level of spatial granularity and temporal immediacy like never before.

</details>


### [12] [Channel Estimation via Tensor Decomposition for Dynamic Metasurface Antennas with Known Mutual Coupling: Algorithms and Experiments](https://arxiv.org/abs/2603.19155)
*Jean Tapie,Bruno Sokal,André L. F. de Almeida,Philipp del Hougne*

Main category: eess.SP

TL;DR: 提出基于张量分解的动态超表面天线信道估计算法，通过广义块Tucker交替最小二乘法处理已知互耦效应，在18GHz原型上实现接近理论上限的精度


<details>
  <summary>Details</summary>
Motivation: 动态超表面天线在实际原型中存在不可忽略的互耦效应，某些架构甚至利用强互耦增强波域灵活性，需要开发能够有效处理已知互耦的信道估计算法

Method: 将已知互耦的信道估计问题建模为张量分解问题，开发广义块Tucker交替最小二乘算法及其变体，针对已知直接和/或馈电信道的情况设计专门版本，并提出互易性感知的双线性分解方法

Result: 在18GHz DMA原型上验证，BTALS算法达到43.1dB精度，仅比实验噪声上限低0.3dB；所有算法均优于现有方案，展示了互耦感知的重要性；进一步应用于双极化3D辐射图预测和信道增益优化

Conclusion: 研究确立了互耦感知张量方法的实际相关性，不仅适用于动态超表面天线，也适用于所有通过可调集总元件实现波域可编程性的无线系统

Abstract: Dynamic metasurface antennas (DMAs) are an emerging hybrid-MIMO technology distinguished by an ultrathin form factor, low cost, and low power consumption. In real-world DMA prototypes, mutual coupling (MC) between meta-elements is generally non-negligible; some architectures even deliberately exploit strong MC to enhance wave-domain flexibility. In this paper, we address channel estimation (CE) for DMAs with known MC by formulating it as a tensor-decomposition problem. We develop a generalized block Tucker alternating least squares (BTALS) algorithm, together with specialized variants for cases with known direct and/or feed channel. We also develop a reciprocity-aware bilinear factorization method for the case with known direct channel. We experimentally validate our algorithms using an 18 GHz DMA prototype whose 7 feeds and 96 meta-elements are strongly coupled via a chaotic cavity. Our general BTALS algorithm reaches an accuracy of 43.1 dB, only 0.3 dB below the upper bound imposed by experimental noise. All proposed algorithms generally outperform the prior-art reference scheme thanks to the superior noise rejection enabled by the tensor-based framework. We further study the minimum number of required measurements as a function of the number of feeds and demonstrate the importance of MC awareness by comparison with an MC-unaware benchmark. Finally, we apply BTALS to a second setup enabling the prediction of the DMA's full dual-polarization 3D radiation diagram. We also measure the latter for DMA configurations optimized for channel-gain enhancement based on the estimated channels. Altogether, our work establishes the practical relevance of MC-aware tensor methods; beyond DMAs, it applies to all wireless systems with wave-domain programmability enabled by tunable lumped elements.

</details>


### [13] [Outage Probability Analysis of NOMA Enabled Hierarchical UAV Networks with Non-Linear Energy Harvesting](https://arxiv.org/abs/2603.19214)
*Faicel Khennoufa,Khelil Abdellatif,Metin Ozturk,Halim Yanikomeroglu,Safwan Alfattani*

Main category: eess.SP

TL;DR: 提出分层无人机自组织网络，结合非线性能量收集和NOMA技术，提升物联网网络的能量和成本效率


<details>
  <summary>Details</summary>
Motivation: 无人机在6G网络中可增强连接性和覆盖范围，但多无人机系统需要更高效的能量管理。能量收集技术对无人机广泛采用至关重要，现有系统在能量效率和实际约束方面存在不足。

Method: 设计两层无人机架构：簇头无人机作为源节点，簇成员无人机作为中继节点，能够从地面功率信标收集能量。采用非线性能量收集和非正交多址接入技术，在Nakagami-m衰落信道下考虑硬件损伤等实际约束，推导了系统中断概率表达式。

Result: 推导了地面物联网设备、每个簇成员无人机以及整个系统的中断概率表达式。与无能量收集系统相比，所提系统在中断概率方面表现更优。

Conclusion: 提出的分层无人机网络结合非线性能量收集和NOMA技术，能够有效提升物联网网络的能量和成本效率，为6G网络中无人机系统的实际部署提供了可行方案。

Abstract: Uncrewed aerial vehicles (UAVs) are expected to enhance connectivity, extend network coverage, and support advanced communication services in sixth-generation (6G) cellular networks, particularly in public and civil domains. Although multi-UAV systems enhance connectivity for IoT networks more than single-UAV systems, energy-efficient communication systems and the integration of energy harvesting (EH) are crucial for their widespread adoption and effectiveness. In this regard, this paper proposes a hierarchical ad hoc UAV network with non-linear EH and non-orthogonal multiple access (NOMA) to enhance both energy and cost efficiency. The proposed system consists of two UAV layers: a cluster head UAV (CHU), which acts as the source, and cluster member UAVs (CMUs), which serve as relays and are capable of harvesting energy from a terrestrial power beacon. For the considered IoT network architecture, the outage probability expressions of ground Internet of things (IoT) devices, each CMU, and the overall outage probability of the proposed system are derived over Nakagami-m fading channels with practical constraints such as hardware impairments and non-linear EH. We compare the proposed system against a non EH system, and our findings indicate that the proposed system outperforms the benchmark in terms of outage probability.

</details>


<div id='eess.AS'></div>

# eess.AS [[Back]](#toc)

### [14] [PCOV-KWS: Multi-task Learning for Personalized Customizable Open Vocabulary Keyword Spotting](https://arxiv.org/abs/2603.18023)
*Jianan Pan,Kejie Huang*

Main category: eess.AS

TL;DR: 提出PCOV-KWS框架，通过轻量网络同时执行关键词检测和说话人验证，实现个性化开放词汇关键词检测，减少参数和计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 随着物联网、语音识别、说话人验证和文本转语音等技术发展，智能语音助手使用增加，对隐私和个性化需求提升，需要解决个性化关键词检测问题。

Method: 采用多任务学习框架，使用轻量网络同时执行关键词检测和说话人验证；引入非softmax的损失函数，将多分类转为多个二分类消除类别竞争；采用多任务损失权重优化策略。

Result: 在多个数据集上评估，PCOV-KWS系统在评估结果上优于基线方法，同时需要更少的参数和更低的计算资源。

Conclusion: 提出的PCOV-KWS框架有效解决了个性化开放词汇关键词检测需求，在性能和效率上均有优势。

Abstract: As advancements in technologies like Internet of Things (IoT), Automatic Speech Recognition (ASR), Speaker Verification (SV), and Text-to-Speech (TTS) lead to increased usage of intelligent voice assistants, the demand for privacy and personalization has escalated. In this paper, we introduce a multi-task learning framework for personalized, customizable open-vocabulary Keyword Spotting (PCOV-KWS). This framework employs a lightweight network to simultaneously perform Keyword Spotting (KWS) and SV to address personalized KWS requirements. We have integrated a training criterion distinct from softmax-based loss, transforming multi-class classification into multiple binary classifications, which eliminates inter-category competition, while an optimization strategy for multi-task loss weighting is employed during training. We evaluated our PCOV-KWS system in multiple datasets, demonstrating that it outperforms the baselines in evaluation results, while also requiring fewer parameters and lower computational resources.

</details>


### [15] [ProKWS: Personalized Keyword Spotting via Collaborative Learning of Phonemes and Prosody](https://arxiv.org/abs/2603.18024)
*Jianan Pan,Yuanming Zhang,Kejie Huang*

Main category: eess.AS

TL;DR: ProKWS是一个新颖的关键词识别框架，通过结合细粒度音素学习和个性化韵律建模来提升系统性能，在标准基准测试中达到SOTA水平并展现出对个性化关键词的强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前关键词识别系统主要依赖音素级匹配来区分易混淆词，但忽略了用户特定的发音特征（如韵律：语调、重音、节奏）。这些个性化特征对于准确识别带有不同语气和意图的关键词至关重要。

Method: 提出ProKWS框架，采用双流编码器：一个流通过对比学习获得鲁棒的音素表示，另一个流提取说话人特定的韵律模式。设计协作融合模块动态结合音素和韵律信息，增强对不同声学环境的适应性。

Result: 实验表明ProKWS在标准基准测试中达到了与最先进模型相当的高度竞争力性能，并对带有语气和意图变化的个性化关键词展现出强大的鲁棒性。

Conclusion: 通过整合细粒度音素学习和个性化韵律建模，ProKWS框架显著提升了关键词识别系统的性能，特别是在处理用户特定发音特征方面表现出色，为个性化语音交互提供了有效解决方案。

Abstract: Current keyword spotting systems primarily use phoneme-level matching to distinguish confusable words but ignore user-specific pronunciation traits like prosody (intonation, stress, rhythm). This paper presents ProKWS, a novel framework integrating fine-grained phoneme learning with personalized prosody modeling. We design a dual-stream encoder where one stream derives robust phonemic representations through contrastive learning, while the other extracts speaker-specific prosodic patterns. A collaborative fusion module dynamically combines phonemic and prosodic information, enhancing adaptability across acoustic environments. Experiments show ProKWS delivers highly competitive performance, comparable to state-of-the-art models on standard benchmarks and demonstrates strong robustness for personalized keywords with tone and intent variations.

</details>


### [16] [ARTT: Augmented Reverberant-Target Training for Unsupervised Monaural Speech Dereverberation](https://arxiv.org/abs/2603.18485)
*Siqi Song,Fulin Wu,Zhong-Qiu Wang*

Main category: eess.AS

TL;DR: 提出两阶段无监督单声道语音去混响方法ARTT：第一阶段通过进一步混响训练网络恢复原始混响信号，第二阶段基于均值教师算法在线自蒸馏提升性能。


<details>
  <summary>Details</summary>
Motivation: 单声道无监督语音去混响由于缺乏干净参考信号和空间线索，是一个具有挑战性的不适定逆问题，需要有效的无监督解决方案。

Method: ARTT包含两个阶段：1）混响目标训练（RTT）- 进一步混响观测信号，训练DNN恢复原始混响信号；2）基于均值教师算法的在线自蒸馏机制，进一步提升去混响效果。

Result: 评估结果表明ARTT实现了强大的无监督去混响性能，显著优于之前的基线方法。

Conclusion: ARTT方法通过创新的两阶段训练策略，成功解决了单声道无监督语音去混响的挑战性问题，为这一领域提供了有效的解决方案。

Abstract: Due to the absence of clean reference signals and spatial cues, monaural unsupervised speech dereverberation is a challenging ill-posed inverse problem. To realize it, we propose augmented reverberant-target training (ARTT), which consists of two stages. In the first stage, reverberant-target training (RTT) is proposed to first further reverberate the observed reverberant mixture signal, and then train a deep neural network (DNN) to recover the observed reverberant mixture via discriminative training. Although the target signal to fit is reverberant, we find that the resulting DNN can effectively reduce reverberation. In the second stage, an online self-distillation mechanism based on the mean-teacher algorithm is proposed to further improve dereverberation. Evaluation results demonstrate that ARTT achieves strong unsupervised dereverberation performance, significantly outperforming previous baselines.

</details>


### [17] [How Auditory Knowledge in LLM Backbones Shapes Audio Language Models: A Holistic Evaluation](https://arxiv.org/abs/2603.19195)
*Ke-Han Lu,Szu-Wei Fu,Chao-Han Huck Yang,Zhehuai Chen,Sung-Feng Huang,Chih-Kai Yang,Yi-Cheng Lin,Chi-Yuan Hsiao,Wenze Ren,En-Pei Hu,Yu-Han Huang,An-Yu Cheng,Cheng-Han Chiang,Yu Tsao,Yu-Chiang Frank Wang,Hung-yi Lee*

Main category: eess.AS

TL;DR: 该研究探讨了大型语言模型在纯文本预训练中获得的听觉知识，并通过三个实验设置评估不同LLM的听觉知识水平及其对音频下游任务的影响。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型常被用作大型音频语言模型的知识骨干，但LLM通过纯文本预训练编码了多少听觉知识，以及这对下游性能的影响尚不清楚。研究旨在填补这一空白。

Method: 采用三种评估设置：1) 在AKB-2000基准上直接探测听觉知识的广度和深度；2) 级联评估，LLM基于音频描述器生成的文本描述进行推理；3) 音频接地评估，将LLM与音频编码器微调为大型音频语言模型。

Result: 研究发现不同模型家族的听觉知识存在显著差异，纯文本评估结果与音频性能强相关。这为理解LLM在音频研究中的作用提供了实证基础。

Conclusion: 该研究为全面理解LLM在音频研究中的角色提供了实证基础，揭示了纯文本预训练获得的听觉知识对音频下游任务的重要性。

Abstract: Large language models (LLMs) have been widely used as knowledge backbones of Large Audio Language Models (LALMs), yet how much auditory knowledge they encode through text-only pre-training and how this affects downstream performance remains unclear. We study this gap by comparing different LLMs under two text-only and one audio-grounded setting: (1) direct probing on AKB-2000, a curated benchmark testing the breadth and depth of auditory knowledge; (2) cascade evaluation, where LLMs reason over text descriptions from an audio captioner; and (3) audio-grounded evaluation, where each LLM is fine-tuned into a Large Audio Language Model (LALM) with an audio encoder. Our findings reveal that auditory knowledge varies substantially across families, and text-only results are strongly correlated with audio performance. Our work provides empirical grounding for a comprehensive understanding of LLMs in audio research.

</details>


<div id='cs.SD'></div>

# cs.SD [[Back]](#toc)

### [18] [MOSS-TTS Technical Report](https://arxiv.org/abs/2603.18090)
*Yitian Gong,Botian Jiang,Yiwei Zhao,Yucheng Yuan,Kuangwei Chen,Yaozhou Jiang,Cheng Chang,Dong Hong,Mingshu Chen,Ruixiao Li,Yiyang Zhang,Yang Gao,Hanfu Chen,Ke Chen,Songlin Wang,Xiaogui Yang,Yuqian Zhang,Kexin Huang,ZhengYuan Lin,Kang Yu,Ziqi Chen,Jin Wang,Zhaoye Fei,Qinyuan Cheng,Shimin Li,Xipeng Qiu*

Main category: cs.SD

TL;DR: MOSS-TTS是一个基于可扩展配方的语音生成基础模型，采用离散音频token、自回归建模和大规模预训练，支持多语言、零样本语音克隆、时长控制、发音控制等功能。


<details>
  <summary>Details</summary>
Motivation: 构建一个可扩展的语音生成基础模型，通过离散音频token、自回归建模和大规模预训练的方法，实现高质量的语音合成，支持多种控制功能和长上下文生成。

Method: 基于MOSS-Audio-Tokenizer（因果Transformer tokenizer，将24kHz音频压缩到12.5fps，使用可变比特率RVQ和统一的语义-声学表示），开发了两个互补的生成器：MOSS-TTS（强调结构简单性和可扩展性）和MOSS-TTS-Local-Transformer（引入帧局部自回归模块以提高建模效率）。

Result: 模型支持多语言和开放域设置，具备零样本语音克隆、token级时长控制、音素/拼音级发音控制、流畅的代码切换和稳定的长格式生成能力。

Conclusion: MOSS-TTS提供了一个可扩展的语音生成基础模型框架，通过创新的tokenizer设计和互补的生成器架构，在保持高质量语音合成的同时，实现了多种控制功能和高效的部署能力。

Abstract: This technical report presents MOSS-TTS, a speech generation foundation model built on a scalable recipe: discrete audio tokens, autoregressive modeling, and large-scale pretraining. Built on MOSS-Audio-Tokenizer, a causal Transformer tokenizer that compresses 24 kHz audio to 12.5 fps with variable-bitrate RVQ and unified semantic-acoustic representations, we release two complementary generators: MOSS-TTS, which emphasizes structural simplicity, scalability, and long-context/control-oriented deployment, and MOSS-TTS-Local-Transformer, which introduces a frame-local autoregressive module for higher modeling efficiency, stronger speaker preservation, and a shorter time to first audio. Across multilingual and open-domain settings, MOSS-TTS supports zero-shot voice cloning, token-level duration control, phoneme-/pinyin-level pronunciation control, smooth code-switching, and stable long-form generation. This report summarizes the design, training recipe, and empirical characteristics of the released models.

</details>


### [19] [Modeling Overlapped Speech with Shuffles](https://arxiv.org/abs/2603.17769)
*Matthew Wiesner,Samuele Cornell,Alexander Polok,Lucas Ondel Yang,Lukáš Burget,Sanjeev Khudanpur*

Main category: cs.SD

TL;DR: 提出使用洗牌积和偏序有限状态自动机（FSA）对重叠语音进行建模，实现单次对齐和说话人归属转录


<details>
  <summary>Details</summary>
Motivation: 解决重叠语音的对齐和说话人归属转录问题，现有方法需要多次对齐，本文旨在实现单次对齐

Method: 使用洗牌积和偏序FSA建模并行数据流，通过总分数作为损失函数，在子词、词和短语级别对所有可能的序列化进行边缘化，加入时间约束减少图大小，直接建模（词元，说话人）元组

Result: 在合成LibriSpeech重叠数据上评估，实现了单次对齐多说话人录音，据所知是首个实现单次对齐的算法

Conclusion: 提出的洗牌积和偏序FSA方法能有效处理重叠语音的对齐和说话人归属转录，实现了单次对齐，所有算法使用k2/Icefall实现

Abstract: We propose to model parallel streams of data, such as overlapped speech, using shuffles. Specifically, this paper shows how the shuffle product and partial order finite-state automata (FSAs) can be used for alignment and speaker-attributed transcription of overlapped speech. We train using the total score on these FSAs as a loss function, marginalizing over all possible serializations of overlapping sequences at subword, word, and phrase levels. To reduce graph size, we impose temporal constraints by constructing partial order FSAs. We address speaker attribution by modeling (token, speaker) tuples directly. Viterbi alignment through the shuffle product FSA directly enables one-pass alignment. We evaluate performance on synthetic LibriSpeech overlaps. To our knowledge, this is the first algorithm that enables single-pass alignment of multi-talker recordings. All algorithms are implemented using k2 / Icefall.

</details>


### [20] [Towards Interpretable Framework for Neural Audio Codecs via Sparse Autoencoders: A Case Study on Accent Information](https://arxiv.org/abs/2603.18359)
*Shih-Heng Wang,Tiantian Feng,Aditya Kommineni,Thanathai Lertpetchpun,Bowen Yi,Xuan Shi,Shrikanth Narayanan*

Main category: cs.SD

TL;DR: 使用稀疏自编码器分析神经音频编解码器如何编码口音信息，发现不同编解码器在稀疏表示中的激活模式和位置编码方式不同


<details>
  <summary>Details</summary>
Motivation: 神经音频编解码器在现代语音系统中广泛应用，但其如何编码语言和副语言信息仍不清楚。提高NAC表示的可解释性对于理解其在敏感应用中的部署至关重要。

Method: 使用稀疏自编码器将密集的NAC表示分解为稀疏、可解释的激活。专注于具有挑战性的副语言属性——口音，并提出量化NAC可解释性的框架。评估了4个NAC模型在16种SAE配置下的表现，使用相对性能指标。

Result: DAC和SpeechTokenizer获得最高的可解释性。发现声学导向的NAC主要在稀疏表示的激活幅度中编码口音信息，而语音导向的NAC更依赖激活位置。低比特率的EnCodec变体显示出更高的可解释性。

Conclusion: 通过稀疏自编码器可以有效地分析神经音频编解码器的表示特性，为理解NAC如何编码副语言信息提供了框架，并揭示了不同编解码器在信息编码策略上的差异。

Abstract: Neural Audio Codecs (NACs) are widely adopted in modern speech systems, yet how they encode linguistic and paralinguistic information remains unclear. Improving the interpretability of NAC representations is critical for understanding and deploying them in sensitive applications. Hence, we employ Sparse Autoencoders (SAEs) to decompose dense NAC representations into sparse, interpretable activations. In this work, we focus on a challenging paralinguistic attribute-accent-and propose a framework to quantify NAC interpretability. We evaluate four NAC models under 16 SAE configurations using a relative performance index. Our results show that DAC and SpeechTokenizer achieve the highest interpretability. We further reveal that acoustic-oriented NACs encode accent information primarily in activation magnitudes of sparse representations, whereas phonetic-oriented NACs rely more on activation positions, and that low-bitrate EnCodec variants show higher interpretability.

</details>


### [21] [Few-shot Acoustic Synthesis with Multimodal Flow Matching](https://arxiv.org/abs/2603.19176)
*Amandine Brunetto*

Main category: cs.SD

TL;DR: FLAC是一种基于流匹配的少样本声学生成方法，能够仅使用最少场景信息生成逼真的房间脉冲响应，在单样本条件下超越现有八样本方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有神经声场方法需要密集音频测量和昂贵的场景特定训练，少样本方法虽然提高了可扩展性，但仍然是确定性的，无法捕捉稀疏上下文下场景声学固有的不确定性。

Method: FLAC采用基于流匹配目标的扩散变换器，在空间、几何和声学线索的条件下，生成新场景中任意位置的房间脉冲响应分布。

Result: 在AcousticRooms和Hearing Anything Anywhere数据集上，FLAC仅使用单样本就超越了最先进的八样本基线方法。同时提出了AGREE评估框架用于几何一致性的声学生成评估。

Conclusion: 这是首次将生成流匹配应用于显式RIR合成，为鲁棒且数据高效的声学合成开辟了新方向。

Abstract: Generating audio that is acoustically consistent with a scene is essential for immersive virtual environments. Recent neural acoustic field methods enable spatially continuous sound rendering but remain scene-specific, requiring dense audio measurements and costly training for each environment. Few-shot approaches improve scalability across rooms but still rely on multiple recordings and, being deterministic, fail to capture the inherent uncertainty of scene acoustics under sparse context. We introduce flow-matching acoustic generation (FLAC), a probabilistic method for few-shot acoustic synthesis that models the distribution of plausible room impulse responses (RIRs) given minimal scene context. FLAC leverages a diffusion transformer trained with a flow-matching objective to generate RIRs at arbitrary positions in novel scenes, conditioned on spatial, geometric, and acoustic cues. FLAC outperforms state-of-the-art eight-shot baselines with one-shot on both the AcousticRooms and Hearing Anything Anywhere datasets. To complement standard perceptual metrics, we further introduce AGREE, a joint acoustic-geometry embedding, enabling geometry-consistent evaluation of generated RIRs through retrieval and distributional metrics. This work is the first to apply generative flow matching to explicit RIR synthesis, establishing a new direction for robust and data-efficient acoustic synthesis.

</details>


### [22] [Words at Play: Benchmarking Audio Pun Understanding in Large Audio-Language Models](https://arxiv.org/abs/2603.18678)
*Yuchen Su,Shaoxin Zhong,Yonghua Zhu,Ruofan Wang,Zijian Huang,Qiqi Wang,Na Zhao,Diana Benavides-Prado,Michael Witbrock*

Main category: cs.SD

TL;DR: APUN-Bench是首个专门评估大型音频语言模型在音频双关语理解能力的基准，包含4,434个音频样本，涵盖双关识别、定位和含义推理三个任务，揭示了现有模型在音频幽默理解方面的显著不足。


<details>
  <summary>Details</summary>
Motivation: 双关语利用多义性和语音歧义创造幽默，对自然语言理解构成独特挑战。音频作为人类交流的核心模态，在双关语研究中缺乏系统数据集和资源，导致这一重要领域未被充分探索。

Method: 构建APUN-Bench基准，包含4,434个音频样本，标注分为三个阶段：双关识别、双关词定位和双关含义推理。系统评估了10个最先进的大型音频语言模型，深入分析其性能差距。

Result: 评估发现现有模型在识别、定位和解释音频双关语方面存在显著性能差距。揭示了音频双关定位中的位置偏见和含义推理中的错误案例等关键挑战。

Conclusion: APUN-Bench为音频双关理解提供了首个系统评估基准，揭示了当前模型的局限性，为推进幽默感知的音频智能提供了可操作的见解和方向。

Abstract: Puns represent a typical linguistic phenomenon that exploits polysemy and phonetic ambiguity to generate humour, posing unique challenges for natural language understanding. Within pun research, audio plays a central role in human communication except text and images, while datasets and systematic resources for spoken puns remain scarce, leaving this crucial modality largely underexplored. In this paper, we present APUN-Bench, the first benchmark dedicated to evaluating large audio language models (LALMs) on audio pun understanding. Our benchmark contains 4,434 audio samples annotated across three stages: pun recognition, pun word location and pun meaning inference. We conduct a deep analysis of APUN-Bench by systematically evaluating 10 state-of-the-art LALMs, uncovering substantial performance gaps in recognizing, localizing, and interpreting audio puns. This analysis reveals key challenges, such as positional biases in audio pun location and error cases in meaning inference, offering actionable insights for advancing humour-aware audio intelligence.

</details>
